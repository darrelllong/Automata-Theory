...
Transitions are given schematically. For each
$d\in\{\pmb{a},\pmb{b},\pmb{c}\}$,
\[
\delta(q_\text{start},d)=(q_d,\overline d,R),
\]
which marks the leftmost unprocessed symbol and records it in the state.  In
state $q_d$, move right over unmarked and marked input symbols until the first
blank is reached; then move one square left into $q_{\text{check},d}$.  In
$q_{\text{check},d}$, accept the pair if the scanned unmarked symbol is $d$ by
writing $\overline d$ and moving left to $q_\text{return}$; reject if it is a
different unmarked input symbol.  State $q_\text{return}$ moves left to the
left end of the tape and then returns to $q_\text{start}$. If
$q_\text{start}$ finds no unmarked input symbol, it enters $q_\text{accept}$.
...
\item Restore the tape, then pair off $\pmb{b}$s and $\pmb{c}$s: erase one
remaining $\pmb{b}$ and one remaining $\pmb{c}$ in each pass. If they run out
simultaneously, $n=m$; reject.
...
\begin{enumerate}
\item \textbf{Construction}: First let $M$ deterministically test whether the
input has equally many $\pmb{a}$'s, $\pmb{b}$'s, and $\pmb{c}$'s:
\begin{enumerate}[1.]
\item Scan from the left to find the first unmarked $\pmb{a}$. If there is none,
check whether any unmarked $\pmb{b}$ or $\pmb{c}$ remains. If not, the test
succeeds; otherwise it fails.
\item Mark that $\pmb{a}$, continue right to find the first unmarked $\pmb{b}$,
and mark it. If no such $\pmb{b}$ exists, the test fails.
\item Continue right to find the first unmarked $\pmb{c}$ and mark it. If no
such $\pmb{c}$ exists, the test fails.
...
\item \textbf{LBA}: Let the input length be $n$. An LBA can use exactly the $n$
input cells and marked versions of the input symbols. First, repeatedly mark the
leftmost unmarked symbol as belonging to the left half and the rightmost
unmarked symbol as belonging to the right half. If the final left mark has no
right-hand mate, then $n$ is odd and the machine rejects. Otherwise the two
halves each have $n/2$ cells. Repeatedly compare the leftmost unprocessed symbol
of the left half with the leftmost unprocessed symbol of the right half, marking
both as processed and rejecting on a mismatch. The machine accepts when every
pair agrees. All marks overwrite original input cells, so no cell outside the
length-$n$ input region is scanned.
...
One synchronization step advances each head, in turn, to its next unmatched
symbol of the appropriate type or to the right end of the input. After all three
advances are complete, accept if all three heads reached the end; reject if
exactly one or two heads reached the end; otherwise begin the next
...
Since the PDA is nondeterministic, $M$ is a nondeterministic TM. A deterministic
TM can simulate such a machine by enumerating all finite computation paths in
lexicographic order and checking them one by one.
...
checks whether the left side of that production occurs at that position, and if
so replaces that occurrence by the right side by shifting the surrounding tape
contents as needed.
...
of guessed productions makes tape 2 agree with tape 1. Thus this two-tape
construction is bounded by a linear amount of tape, and hence is a
...
- Every ordinary move of $M$ is matched by the corresponding move of $M'$ while
preserving the marked end cells.
...
rightmost cell.'' The modified strict LBA duplicates every transition that
previously referred to $\overline{d}$ or $\underline{d}$, replacing those marked
symbols by $\overline{\underline{d}}$ whenever the word has length $1$. In this
way the machine still knows that the scanned cell is simultaneously the left and
right boundary cell.
...
strict machine uses three tracks: one track stores the tape symbols, one marks
the left end of the input, and one marks the right end. The transition function
is changed so that whenever the old machine would step onto an endmarker and
then return, the new machine performs the corresponding detour entirely inside
the marked input cells. Thus every type~1
language can be recognized by a
...
\item \textbf{Correct $G_*$}: A separator alone is insufficient if it may be
erased before its neighboring derivations are complete. Instead, simulate each
copy of $G$ using a tagged copy of every symbol. For each
$X\in\Sigma\cup\Gamma$, choose a fresh nonterminal $\widehat X$, and for a word
$\alpha=X_1\cdots X_r$ write
$\widehat\alpha=\widehat X_1\cdots\widehat X_r$. Let
\[
\widehat P=\{\widehat\alpha\to\widehat\beta\mid
\alpha\to\beta\in P\}.
\]
Choose fresh nonterminals $W$ and $\#$, and define
\[
G_*=\langle\Sigma,\{\widehat X\mid X\in\Sigma\cup\Gamma\}\cup\{W,\#\},W,P_*\rangle,
\]
where $P_*$ contains $\widehat P$ together with
\[
W\to\lambda\mid\widehat Z\mid\widehat Z\#W,\qquad
\widehat a\to a\quad(a\in\Sigma),
\]
and the sole separator-removal rules
\[
a\#b\to ab\quad(a,b\in\Sigma).
\]
The rule $a\#\to a$ is deliberately absent: the final copied word is created by
$W\to\widehat Z$, so no trailing separator has to be erased. An internal
separator can disappear only when a terminal symbol occurs on both sides. Even
if the rest of the right-hand copy has not yet finished, its first untagged
terminal symbol blocks every production in $\widehat P$, whose symbols are all
tagged. Thus tagged productions cannot cross either a separator or an untagged
terminal symbol.

\item \textbf{Proof}: Let
\[
w=w_1w_2\cdots w_k\qquad\text{with each }w_i\in L(G)-\{\lambda\}.
\]
For $k>0$, start with $W\to\widehat Z\#W$ exactly $k-1$ times and then use
$W\to\widehat Z$. In each tagged block, use $\widehat P$ to simulate a derivation
$Z\DRightarrow_G w_i$, then apply $\widehat a\to a$ to expose the terminal
letters of $w_i$. Once terminal symbols appear on both sides of each boundary,
the rules $a\#b\to ab$ remove the resulting separators, giving $w$. The
zero-factor derivation gives $\lambda$. Since
$(L(G)-\{\lambda\})^*=L(G)^*$, this proves $L(G)^*\subseteq L(G_*)$.

Conversely, consider a terminal derivation from $W$. Every tagged block is
created by one of the two nonempty alternatives for $W$. In a terminal result,
the tagged nonterminals in each block have been eliminated by rules from
$\widehat P$, and every remaining tagged terminal has been converted by a rule
$\widehat a\to a$. If the tags are erased from this sequence of steps, each
$\widehat P$ step is a production of $G$ and each conversion step changes
nothing, so the resulting nonempty terminal word belongs to $L(G)$.

A separator can be removed only by $a\#b\to ab$. At that moment the untagged
letters $a$ and $b$ remain between the two copies, so no tagged production can
ever cross their former boundary, even if an unfinished tagged substring occurs
farther to the right. Therefore every terminal word derived from $W$ is a finite
concatenation of words in $L(G)$, and
$L(G_*)\subseteq L(G)^*$. Hence $L(G_*)=L(G)^*$. \qed
\end{enumerate}
...
input $y$, list the finitely many splits $y=uv$. Simulate every associated pair
$(M_1,u)$ and $(M_2,v)$ in round-robin fashion: at stage $t$, perform one more
step of each simulation that has not halted. Accept as soon as both simulations
for one split have accepted. If $y\in L_1L_2$, the accepting split eventually
gets enough simulated steps for both machines to accept; if no split works, the
machine need not halt. Thus this semidecides $L_1L_2$.
...
$\pmb{Y}$ somewhere on the tape. So $L_3(A_3)=L_2(A_2)=L$.

\item \textbf{$L_3(A_3)$ to $L_1(A_1)$}: Let $A_3$ be a machine with
$L=L_3(A_3)$. Redirect every transition entering the halt state of $A_3$ to a
new verification routine. The routine scans right to the first blank cell,
steps left, and checks the last nonblank tape symbol. If that symbol is
$\pmb{Y}$, it enters a new halt state; otherwise it enters a simple infinite
loop. Hence $A_1$ halts exactly when $A_3$ halts with $\pmb{Y}$ at the right end
of its tape. Therefore $L_1(A_1)=L_3(A_3)=L$. \qed
\end{enumerate}
...
\item \textbf{Inverse homomorphism}: On input $x$, first compute $h(x)$ and then
run the halting machine for $L$ on that word. This composed machine halts because
the preliminary computation of $h(x)$ is finite and the machine for $L$ halts on
every input. It accepts exactly when $h(x)\in L$, which is exactly when
$x\in h^{-1}(L)$. Therefore $h^{-1}(L)\in\OSIG$.
...
$y_i\in s(\pmb{a}_i)$ for every $i$; accept iff (at least) one choice works. Hence
