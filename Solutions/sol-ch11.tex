...
\[
W\to\lambda\mid\widehat Z\#W,\qquad
\widehat a\to a\quad(a\in\Sigma),
\]
and the separator-removal rules
\[
a\#\to a\quad(a\in\Sigma),\qquad
a\#b\to ab\quad(a,b\in\Sigma).
\]
Thus $\#$ is never erased by itself: it can disappear only after a completed
terminal word, and an internal separator also requires a completed terminal word
on its right. The tagged productions cannot cross either a separator or an
untagged terminal symbol.

\item \textbf{Proof}: Let
\[
w=w_1w_2\cdots w_k\qquad\text{with each }w_i\in L(G)-\{\lambda\}.
\]
Starting with $W$, use $W\to\widehat Z\#W$ exactly $k$ times and then
$W\to\lambda$. In each tagged block, use $\widehat P$ to simulate a derivation
$Z\DRightarrow_G w_i$, then apply $\widehat a\to a$ to expose the terminal
letters of $w_i$. The separator rules remove the resulting separators, giving
$w$. The zero-factor derivation gives $\lambda$. Since
$(L(G)-\{\lambda\})^*=L(G)^*$, this proves $L(G)^*\subseteq L(G_*)$.

Conversely, consider a terminal derivation from $W$. Every tagged block is
created by $W\to\widehat Z\#W$. Before such a block can contribute only terminal
letters, its tagged nonterminals must have been eliminated by rules from
$\widehat P$, and each remaining tagged terminal must have been converted by a
rule $\widehat a\to a$. Those steps correspond to a derivation in $G$ of the
same nonempty terminal word. A separator can be removed only between terminal
letters (or at the end of the last terminal word), so no tagged production can
ever cross from one block into another. Therefore every terminal word derived
from $W$ is a finite concatenation of words in $L(G)$, and
