...
\]
The final factor need only be $\epsilon\cup\pmb{c}$, because any terminal
non-$\pmb{c}$ letter is already covered by the last block chosen from
$N\cup\pmb{c}N$.
...
A nonempty decimal string represents a number divisible by 5 iff it ends in
$\pmb{0}$ or $\pmb{5}$.  Using the same convention as Exercise~6.12, where
$\lambda$ represents zero,
\[
...
We follow the unsimplified Chapter~5 constructions literally, without merging
states that happen to be equivalent.
...
\]
Since $\lambda\notin(\pmb{10}\cup\pmb{11}\cup\pmb{0})$, Theorem~6.1 gives
\[
...
\begin{enumerate}[1.]
  ...
  \item Convert $R_2$ to an NDFA $A_2$ so that $L(A_2)=L(R_2)$.
...
\item \begin{enumerate}[(a)]
  ...
  $X_2=\pmb{c}\cup\emptyset\cdot X_1\cup\epsilon\cdot X_2=\pmb{c}\cup X_2$.  Thus any $X_2\supseteq\{\pmb{c}\}$ is a solution.

  \item \textbf{No contradiction with Theorem~6.2:}  Theorem~6.2 assumes
  $\lambda\notin A_{ij}$ for \emph{every} coefficient in the system.  Here
...
\begin{enumerate}[(a)]
  ...
  \emph{Step:}  Assume the result for all expressions with $<m$ operators.
  If $R=R_1\cup R_2$, then
  \[
  (L(R_1)\cup L(R_2))^r=L(R_1)^r\cup L(R_2)^r
  =L(R_1^r)\cup L(R_2^r)=L(R_1^r\cup R_2^r)=L(R^r).
  \]
  If $R=R_1\cdot R_2$, then
  \[
  L(R)^r=(L(R_1)L(R_2))^r=L(R_2)^rL(R_1)^r
  =L(R_2^r)L(R_1^r)=L(R_2^r\cdot R_1^r)=L(R^r).
  \]
  If $R=R_1^*$, then
  \[
  L(R)^r=(L(R_1)^*)^r=(L(R_1)^r)^*
  =(L(R_1^r))^*=L((R_1^r)^*)=L(R^r).
  \]
  Thus the claim holds in all three operator cases.  \qed
...
\item \begin{enumerate}[(a)]
  ...
  \]
  The right-hand term simplifies, since
  \[
    \epsilon\cup\pmb{a}\pmb{a}^*=\pmb{a}^*.
  \]
  So a shorter equivalent expression is
  \[
    (\pmb{b}\cup\pmb{c}\cup\pmb{a}\pmb{a}^*\pmb{c})^*\pmb{a}^*.
  \]
...
\item \begin{enumerate}[(a)]
  ...
  $\pmb{a}$ or occurs at the end of the word.  Hence one regular expression is
	  ...
	  \]
	  Since $\epsilon\cup\pmb{c}\pmb{c}^*=\pmb{c}^*$, this may also be written more
	  compactly as
	  \[
	    (\pmb{a}\cup\pmb{b}\cup\pmb{c}\pmb{c}^*\pmb{a})^*\pmb{c}^*.
	  \]

	  \item Words with exactly one $\pmb{c}$ and no $\pmb{cb}$:

  Exactly one $\pmb{c}$, and that $\pmb{c}$ is not immediately followed by
  $\pmb{b}$, means that the lone $\pmb{c}$ is followed by either $\pmb{a}$
  and then any string over $\{\pmb{a},\pmb{b}\}$, or by the end of the word:
  \[
...
\item \begin{enumerate}[(a)]
  \item For Figure~6.11 the right-linear equations are
	  ...
	  \item In the notation of Theorem~6.2,
	  \[
	  E_1=\emptyset,\quad E_2=\epsilon,\quad
	  A_{11}=\pmb{a},\quad A_{12}=\pmb{a},\quad
	  A_{21}=\pmb{a}\cup\pmb{b},\quad A_{22}=\emptyset.
	  \]
	  Eliminating $X_2$ gives the one-variable equation
	  \[
	  ...
	  \[
	  \hat{E}_1
	  =E_1\cup A_{12}A_{22}^*E_2
	  =\emptyset\cup\pmb{a}\emptyset^*\epsilon
	  =\pmb{a},
	  \]
	  and
	  \[
	  \hat{A}_{11}
	  =A_{11}\cup A_{12}A_{22}^*A_{21}
	  =\pmb{a}\cup\pmb{a}\emptyset^*(\pmb{a}\cup\pmb{b})
	  =\pmb{a}\cup\pmb{aa}\cup\pmb{ab}.
	  \]
	  By Theorem~6.1,
	  \[
	  X_1=(\pmb{a}\cup\pmb{aa}\cup\pmb{ab})^*\pmb{a}.
	  \]
	  Back-substitution, equivalently Theorem~6.2(c), gives
	  \[
	  X_2=\emptyset^*(\epsilon\cup(\pmb{a}\cup\pmb{b})X_1)
	     =\epsilon\cup(\pmb{a}\cup\pmb{b})(\pmb{a}\cup\pmb{aa}\cup\pmb{ab})^*\pmb{a}.
	  \]

	  The same result follows by direct substitution. Substitute the second equation into the first:
	  \[
...
\item \begin{enumerate}[(a)]
  ...
  \begin{align*}
    ...
    X_2 &= \epsilon \cup \pmb{b}X_2 \cup \pmb{a}X_3,\\
	  ...
	  \item First apply Theorem~6.2 to eliminate $X_3$.  The last equation has
	  $E_3=\emptyset$, $A_{30}=\pmb{b}$, and
	  $A_{33}=\pmb{a}\cup\pmb{b}$, so
	  \[
	  X_3=(\pmb{a}\cup\pmb{b})^*\pmb{b}X_0,
	  \]
	  and the $X_2$ equation becomes
	  \[
	  X_2=\pmb{b}^*(\epsilon\cup\pmb{a}X_3)
	      =\pmb{b}^*\cup\pmb{b}^*\pmb{a}(\pmb{a}\cup\pmb{b})^*\pmb{b}X_0,
	  \]
	  after applying Theorem~6.1 to the coefficient $\pmb{b}$ of $X_2$.

	  Substituting this expression for $X_2$ leaves the pair
	  \begin{align*}
	    X_0 &= \pmb{a}X_1,\\
	    X_1 &= \pmb{a}\pmb{b}^*
	      \cup \pmb{a}\pmb{b}^*\pmb{a}(\pmb{a}\cup\pmb{b})^*\pmb{b}X_0
	      \cup \pmb{a}X_1 .
	  \end{align*}
	  For this pair,
	  \[
	  E_0=\emptyset,\quad
	  E_1=\pmb{a}\pmb{b}^*,\quad
	  A_{00}=\emptyset,\quad A_{01}=\pmb{a},\quad
	  A_{10}=\pmb{a}\pmb{b}^*\pmb{a}(\pmb{a}\cup\pmb{b})^*\pmb{b},\quad
	  A_{11}=\pmb{a}.
	  \]
	  Theorem~6.2 therefore reduces the pair to
	  \[
	  X_0=\hat{E}_0\cup\hat{A}_{00}X_0,
	  \]
	  where
	  \[
	  \hat{E}_0
	  =E_0\cup A_{01}A_{11}^*E_1
	  =\pmb{a}\pmb{a}^*\pmb{a}\pmb{b}^*
	  \]
	  and
	  \[
	  \hat{A}_{00}
	  =A_{00}\cup A_{01}A_{11}^*A_{10}
	  =\pmb{a}\pmb{a}^*\pmb{a}\pmb{b}^*\pmb{a}(\pmb{a}\cup\pmb{b})^*\pmb{b}.
	  \]
	  By Theorem~6.1,
	  \[
	  X_0=
	  ...
	  \pmb{a}\pmb{a}^*\pmb{a}\pmb{b}^*.
	  \]
	  The remaining unknowns are obtained by back-substitution:
	  \[
	  X_1=\pmb{a}^*(\pmb{a}\pmb{b}^*
	      \cup \pmb{a}\pmb{b}^*\pmb{a}(\pmb{a}\cup\pmb{b})^*\pmb{b}X_0),
	  \]
	  \[
	  X_2=\pmb{b}^*(\epsilon\cup\pmb{a}(\pmb{a}\cup\pmb{b})^*\pmb{b}X_0),
	  \qquad
	  X_3=(\pmb{a}\cup\pmb{b})^*\pmb{b}X_0.
	  \]

	  The direct substitution method gives the same equation for $X_0$:
	  \[
	  X_1=\pmb{a}^*\pmb{a}X_2,
	  \qquad
...
\item \begin{enumerate}[(a)]
  ...
  Definition~6.5, we prove directly by structural induction on a regular
  expression $R$ that $\bar{s}(L(R))$ is regular over $\Gamma$.
...
\item \begin{enumerate}[(a)]
  ...
  \item Every $\pmb{b}$ immediately followed by $\pmb{c}$:
  Any valid word is a concatenation of blocks from
  $\{\pmb{a},\pmb{c},\pmb{bc}\}$, because every occurrence of $\pmb{b}$ must be
  paired immediately with a following $\pmb{c}$. Conversely, any concatenation
  of those blocks has the required property. Therefore
  \[
    (\pmb{a}\cup\pmb{c}\cup\pmb{bc})^*
  \]
\end{enumerate}
...
\begin{enumerate}[(a)]
  ...
  \]
	  ...
	  \[
	    \pmb{a} \cup \pmb{b} \cup \pmb{a}(\pmb{a}\cup\pmb{b})^*\pmb{a} \cup \pmb{b}(\pmb{a}\cup\pmb{b})^*\pmb{b}
	  \]
	\end{enumerate}
...
%% 6.50
	...
	\begin{enumerate}[(a)]
	  \item Let $X_0$ denote the terminal set from $t_0$.  The equation is
	  \[
	  X_0=\epsilon\cup(\pmb{a}\cup\pmb{c})X_0.
	  \]
	  Theorem~6.1 gives
	  \[
	  X_0=(\pmb{a}\cup\pmb{c})^*.
	  ...
	  \item Let $X_1$ and $X_0$ denote the terminal sets from $q_1$ and $q_0$,
	  respectively.  The equations are
	  \[
	  X_1=\epsilon\cup\pmb{b}X_1\cup\pmb{a}X_0,
	  \qquad
	  X_0=\pmb{c}X_0\cup\pmb{b}X_1.
	  \]
	  The second equation gives $X_0=\pmb{c}^*\pmb{b}X_1$.  Hence
	  \[
	  X_1=\epsilon\cup(\pmb{b}\cup\pmb{a}\pmb{c}^*\pmb{b})X_1,
	  \]
	  and
	  \[
	  X_1=(\pmb{b}\cup\pmb{a}\pmb{c}^*\pmb{b})^*.
	  \]

	  \item With terminal sets $X_0$ and $X_1$ from $r_0$ and $r_1$,
	  \[
	  X_0=\epsilon\cup\pmb{a}X_0\cup(\pmb{a}\cup\pmb{c})X_1,
	  \qquad
	  X_1=\pmb{b}X_0.
	  \]
	  Substitution gives
	  \[
	  X_0=\epsilon\cup(\pmb{a}\cup(\pmb{a}\cup\pmb{c})\pmb{b})X_0,
	  \]
	  so
	  \[
	  X_0=(\pmb{a}\cup(\pmb{a}\cup\pmb{c})\pmb{b})^*.
	  \]

	  \item With $X_i$ the terminal set from $s_i$,
	  \[
	  X_0=\pmb{a}X_1,\qquad
	  X_1=\pmb{a}X_2\cup\pmb{b}X_3,\qquad
	  X_2=\epsilon,\qquad
	  X_3=\pmb{b}X_1.
	  \]
	  Thus $X_1=\pmb{a}\cup\pmb{bb}X_1$, so
	  $X_1=(\pmb{bb})^*\pmb{a}$ and
	  \[
	  X_0=\pmb{a}(\pmb{bb})^*\pmb{a}.
	  \]

	  \item With terminal sets $X_1$ and $X_2$ from the two states,
	  \[
	  X_1=\pmb{a}X_1\cup\pmb{a}X_2,\qquad
	  X_2=\epsilon\cup(\pmb{a}\cup\pmb{b})X_1.
	  \]
	  ...
	  \[
	  X_0=\pmb{a}X_1,\qquad
	  X_1=X_0\cup\pmb{b}X_2,\qquad
	  X_2=\pmb{c}X_3,\qquad
	  X_3=\epsilon\cup X_1.
	  \]
	  Hence
	  \[
	  X_1=X_0\cup\pmb{bc}\cup\pmb{bc}X_1
	      =\pmb{a}X_1\cup\pmb{bc}\cup\pmb{bc}X_1,
	  \]
	  so
	  \[
	  X_1=(\pmb{a}\cup\pmb{bc})^*\pmb{bc}.
	  \]
	  Since $X_0=\pmb{a}X_1$,
	  \[
	  X_0=\pmb{a}(\pmb{a}\cup\pmb{bc})^*\pmb{bc}
	      =\pmb{a}\pmb{a}^*\pmb{b}\pmb{c}(\pmb{a}^*\pmb{b}\pmb{c})^*.
	  \]

	  \item With $X_i$ the terminal set from $S_i$,
	  \[
	  X_0=\epsilon\cup\pmb{a}X_1,\qquad
	  X_1=\pmb{a}X_0\cup\pmb{c}X_2\cup\pmb{b}X_3,\qquad
	  X_2=\pmb{c}X_1,\qquad
	  X_3=\pmb{b}X_1.
	  \]
	  Substituting for $X_2$ and $X_3$ gives
	  \[
	  X_1=\pmb{a}X_0\cup(\pmb{bb}\cup\pmb{cc})X_1,
	  \]
	  so
	  \[
	  X_1=(\pmb{bb}\cup\pmb{cc})^*\pmb{a}X_0.
	  \]
	  Then
	  \[
	  X_0=\epsilon\cup\pmb{a}(\pmb{bb}\cup\pmb{cc})^*\pmb{a}X_0,
	  \]
	  and therefore
	  \[
	  X_0=\bigl(\pmb{a}(\pmb{bb}\cup\pmb{cc})^*\pmb{a}\bigr)^*.
	  \]
	\end{enumerate}
...
Theorem~6.6 proves closure of \RSIG\ under substitution.  Below are the three
operator cases written out explicitly.

\begin{proof}
The basis step is the case in which $R$ has no operators.  Then $R$ is either a
single letter $\pmb{a}\in\Sigma$, $\emptyset$, or $\epsilon$.  By Definition~6.5,
$s(\pmb{a})$ is a regular expression over $\Gamma$ representing
$\overline{s}(\{\pmb{a}\})$, while $\emptyset$ represents
$\overline{s}(\emptyset)$ and $\epsilon$ represents
$\overline{s}(\{\lambda\})$.  Thus the substituted expression has the required
language in each basis case.
