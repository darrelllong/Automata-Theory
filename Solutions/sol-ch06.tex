...
\item \begin{enumerate}[(a)]
  ...
  $s(\pmb{b})=(\pmb{a}\cup\pmb{b})$.  Then $\overline{s}(L)$ is the set of all
  even-length words over $\{\pmb{a},\pmb{b}\}$, which is regular.  So the image
  of a nonregular language under a substitution need not remain in
  $\mathfrak{N}_\Gamma$.
\end{enumerate}
...
language $\{\pmb{a},\pmb{b}\}$.  Define a substitution by
$s(\pmb{a})=R_1$ and $s(\pmb{b})=R_2$.  Then
\[
\overline{s}(\{\pmb{a},\pmb{b}\})=L_1\cup L_2,
\]
so $L_1\cup L_2$ is regular by Theorem~6.6.

For concatenation, use the regular language $\{\pmb{ab}\}$ with the same
substitution.  Then
\[
\overline{s}(\{\pmb{ab}\})=L_1\cdot L_2,
\]
so $L_1\cdot L_2$ is regular.

For Kleene closure, work over the alphabet $\{\pmb{a}\}$ and use the regular
language $\{\pmb{a}\}^*$.  Define $s(\pmb{a})=R_1$.  Then
\[
\overline{s}(\{\pmb{a}\}^*)=L_1^*,
\]
so $L_1^*$ is regular.
