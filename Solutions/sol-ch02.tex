...
\item The original choice of final states, $[\lambda]_Q\cup C_e$, does not
give a three-state minimal machine: it collapses $[\lambda]_Q$ and $C_e$ and
therefore gives the two-state machine $A_{R_L}$.  To make $A_{R_K}$ look like
the three-state machine $A_Q$, relabel the final states so that
\[
K=C_e.
\]
Then $z=\lambda$ distinguishes $[\lambda]_Q$ from $C_e$, since
$\lambda\notin K$ but $\pmb{aa}\in K$, and it also distinguishes $C_e$ from
$C_o$, since $\pmb{aa}\in K$ but $\pmb{a}\notin K$.  Thus the three
$Q$-classes remain distinct, so $R_K=Q$ and $A_{R_K}$ has the same state
structure as this relabeled $A_Q$.
...
The step below uses the right-end form of the recursion, justified by the
Chapter 1 concatenation theorem for $\DBAR$.
...
We use Theorem 2.5. Suppose, for contradiction, that $L$ is FAD, and let $n$
be the number supplied by that theorem.

Take
\[
x=\mathbf{a}^n\mathbf{b}^{n+1}.
\]
Then $x\in L$, because $|x|_\mathbf{a}=n<n+1=|x|_\mathbf{b}$.

By Theorem 2.5 there should be a decomposition
\[
x=uvw,\qquad |vw|\le n,\qquad |v|\ge1,
\]
such that
\[
uv^iw\in L\qquad\text{for every }i\ge0.
\]

Since the last $n$ symbols of $x$ are all $\mathbf{b}$'s, the substring $v$
must be of the form $\mathbf{b}^k$ for some $k\ge1$.
Now take $i=0$. Then
\[
uw=\mathbf{a}^n\mathbf{b}^{n+1-k}.
\]
Because $k\ge1$, we have $n+1-k\le n$, so
\[
|uw|_\mathbf{b}\le |uw|_\mathbf{a}.
\]
Hence $uw\notin L$, contradicting Theorem 2.5.

Therefore $L$ is not FAD. $\square$

%%------------------------------------------------------------
\item \textbf{Show $G=\{x\mid|x|_\mathbf{a}\geq|x|_\mathbf{b}\}$ is not FAD.}

We use Theorem 2.6. Suppose, for contradiction, that $G$ is FAD, and let $n$
be the number supplied by that theorem.
...
x=\mathbf{a}^n\mathbf{b}^{n+1}.
\]
Then $x\notin G$, because $|x|_\mathbf{a}=n$ and $|x|_\mathbf{b}=n+1$.
...
such that
\[
uv^iw\notin G\qquad\text{for every }i\ge0.
\]

Since the first $n$ symbols of $x$ are all $\mathbf{a}$'s, the substring $v$
must be of the form $\mathbf{a}^k$ for some $k\ge1$.
Now pump once more than usual, taking $i=2$. Then
\[
uv^2w=\mathbf{a}^{n+k}\mathbf{b}^{n+1}.
\]
Because $k\ge1$, we have $n+k\ge n+1$, so
\[
|uv^2w|_\mathbf{a}\ge |uv^2w|_\mathbf{b}.
\]
Hence $uv^2w\in G$, contradicting Theorem 2.6.

Therefore $G$ is not FAD. $\square$
...
such that
\[
uv^iw\notin\Gamma\qquad\text{for every }i\ge0.
\]
...
We use Theorem 2.3. Suppose, for contradiction, that $K$ is FAD, and let $n$
be the number supplied by that theorem.

Take
\[
x=\mathbf{0}^n\mathbf{1}\mathbf{0}^n.
\]
This word is in $K$, since it is the same read from left to right and right to
left.

By Theorem 2.3 there should be a decomposition
\[
x=uvw,\qquad |uv|\le n,\qquad |v|\ge1,
\]
such that
\[
uv^iw\in K\qquad\text{for every }i\ge0.
\]

Since the first $n$ symbols of $x$ are all $\mathbf{0}$'s, the substring $v$
must be of the form $\mathbf{0}^k$ for some $k\ge1$.
Now take $i=0$. Then
\[
uw=\mathbf{0}^{n-k}\mathbf{1}\mathbf{0}^n.
\]
The block of $\mathbf{0}$'s to the left of $\mathbf{1}$ has length $n-k$,
while the block to the right has length $n$. These are unequal, so $uw$ is not
a palindrome. Thus $uw\notin K$, contradicting Theorem 2.3.

Therefore $K$ is not FAD. $\square$
...
such that
\[
uv^iw\notin C\qquad\text{for every }i\ge0.
\]
...
As above, the right-end decomposition is legitimate by the Chapter 1
concatenation theorem for $\DBAR$.
