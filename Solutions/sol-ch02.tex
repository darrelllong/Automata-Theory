...
\]
Hence $uw\not in L$, contradicting Theorem 2.5.
...
x=\mathbf{a}^n\mathbf{b}^{n+1}.
\]
Then $x\not in G$, because $|x|_\mathbf{a}=n$ and $|x|_\mathbf{b}=n+1$.
...
such that
\[
uv^iw\not in G\qquad\text{for every }i\ge0.
\]
...
a palindrome. Thus $uw\not in K$, contradicting Theorem 2.3.
