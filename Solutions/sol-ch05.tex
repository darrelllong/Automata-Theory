...
\begin{enumerate}[(a)]
  ...
  \item \textbf{Proof.}  We prove by induction on $|x|$ that
  \[
  \overline{\delta^-}(\langle s^1_0,s^2_0\rangle,x)
  =\langle\deltabar{1}(s^1_0,x),\deltabar{2}(s^2_0,x)\rangle.
  \]
  For $x=\lambda$ both sides are $\langle s^1_0,s^2_0\rangle$. If the claim holds
  for $x$, then for any letter $\pmb{a}$,
  \[
  \overline{\delta^-}(\langle s^1_0,s^2_0\rangle,x\pmb{a})
  =\delta^-(\overline{\delta^-}(\langle s^1_0,s^2_0\rangle,x),\pmb{a})
  \]
  \[
  =\delta^-(\langle\deltabar{1}(s^1_0,x),\deltabar{2}(s^2_0,x)\rangle,\pmb{a})
  =\langle\deltabar{1}(s^1_0,x\pmb{a}),\deltabar{2}(s^2_0,x\pmb{a})\rangle.
  \]
  Therefore $x\in L(A^-)$ iff $\deltabar{1}(s^1_0,x)\in F_1$ and
  $\deltabar{2}(s^2_0,x)\not in F_2$, which is exactly the condition
