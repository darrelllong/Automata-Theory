...
\begin{enumerate}[(a)]
  ...
  \item \textbf{Concatenation: Yes.}  If $L_1,L_2\in C_\Sigma$, then there exists $N_1$ such that all words of length $\ge N_1$ are in $L_1$, and there exists $N_2$ such that all words of length $\ge N_2$ are in $L_2$.  Then any word of length $\ge N_1+N_2$ can be split so that both parts are in $L_1$ and $L_2$, so $L_1\cdot L_2$ is cofinite.
...
\begin{enumerate}[(a)]
  \item \textbf{Complementation: No.}  $L\in J_\Sigma$ means $\sim L$ is infinite.  For $\sim L$ itself to belong to $J_\Sigma$, its complement $\sim(\sim L)=L$ would also have to be infinite; this need not hold.  Counterexample: $L=\emptyset\in J_\Sigma$ (complement is $\Sigma^*$, infinite), but $\sim L=\Sigma^*\notin J_\Sigma$ (complement is $\emptyset$, finite).  \textbf{Not closed.}
...
\]
Then $K\in\NSIG$. For any word $x$, prefix enough copies of the deficient symbol
to balance the counts; this shows $x\in S(K)$. Hence
\[
S(K)=\{\pmb{a},\pmb{b}\}^*.
\]
Since $S(K)$ is FAD, \NSIG\ is not closed under $S$.
...
K=\{x\in\{\pmb{a},\pmb{b}\}^*\mid |x|_{\pmb{a}}=|x|_{\pmb{b}}\}.
\]
Every word in $K$ has a proper extension in $K$:
...
\begin{enumerate}[(a)]
  ...
  \item \textbf{Proof.}  We prove the stronger claim, by induction on $|x|$,
  that for all $s_1\in S_1$ and $s_2\in S_2$,
  \[
  \overline{\delta^-}(\langle s_1,s_2\rangle,x)
  =\langle\deltabar{1}(s_1,x),\deltabar{2}(s_2,x)\rangle.
  \]
  For $x=\lambda$ both sides are $\langle s_1,s_2\rangle$. If the claim holds
  for $x$, then for any letter $\pmb{a}$, Definition~1.11iii gives
  \[
  \overline{\delta^-}(\langle s_1,s_2\rangle,\pmb{a}x)
  =\overline{\delta^-}(\delta^-(\langle s_1,s_2\rangle,\pmb{a}),x)
  \]
  \[
  =\overline{\delta^-}(\langle\delta_1(s_1,\pmb{a}),\delta_2(s_2,\pmb{a})\rangle,x)
  =\langle\deltabar{1}(\delta_1(s_1,\pmb{a}),x),\deltabar{2}(\delta_2(s_2,\pmb{a}),x)\rangle
  =\langle\deltabar{1}(s_1,\pmb{a}x),\deltabar{2}(s_2,\pmb{a}x)\rangle.
  \]
  In particular, the claim holds for $\langle s^1_0,s^2_0\rangle$.
  Therefore $x\in L(A^-)$ iff $\deltabar{1}(s^1_0,x)\in F_1$ and
  $\deltabar{2}(s^2_0,x)\notin F_2$, which is exactly the condition
  $x\in L_1\setminus L_2$. \qed
\end{enumerate}
...
\item \begin{enumerate}[(a)]
  ...
  \item \textbf{\NSIG\ is not closed under Kleene closure.}  For
  \[
    L=\{\pmb{a},\pmb{b}\}^*-\{\pmb{a}^n\pmb{b}^n\mid n\ge 1\},
  \]
  we have $\lambda\in L$ and $L\cdot L=\Sigma^*$, so
  $L^*=\Sigma^*\in\DSIG$.

  \item \textbf{If $L\in\NSIG$, then $L^+\in\NSIG$: No.}  For
  \[
    L=\{\pmb{a},\pmb{b}\}^*-\{\pmb{a}^n\pmb{b}^n\mid n\ge 1\},
  \]
  because $\lambda\in L$ we have $L^+=L^*=\Sigma^*$, which is FAD.
\end{enumerate}

%% 5.52
\item \textbf{Why require $S_1\cap S_2=\emptyset$ in Theorem~5.4?}

If $S_1\cap S_2\neq\emptyset$, the transition function $\delta^\bullet$ would be ambiguous: for a state $s\in S_1\cap S_2$, which transitions should apply---those of $A_1$ or $A_2$?  The definition of $\delta^\bullet$ would not be well-defined.  Moreover, the proof that paths through the concatenation machine correctly simulate $A_1$ then $A_2$ relies on the fact that once the computation enters $S_2$, it stays in $S_2$, and once it begins in $S_1$ it follows only the $A_1$ transitions until such a handoff occurs.  If the state sets overlap, a path might inadvertently ``mix'' transitions from $A_1$ and $A_2$.
...
\item \textbf{Prove $\overline{\delta^\cap}(\langle s,t\rangle,x)=\langle\DBAR_1(s,x),\DBAR_2(t,x)\rangle$.}
  ...
  \begin{proof}
  We prove the stronger statement, for all state pairs $\langle s,t\rangle$, by
  induction on $|x|$.

  \emph{Base:}  $\overline{\delta^\cap}(\langle s,t\rangle,\lambda)=\langle s,t\rangle=\langle\DBAR_1(s,\lambda),\DBAR_2(t,\lambda)\rangle$.

  \emph{Step:}  Let the string be $\pmb{a}x$, where $\pmb{a}$ is a single letter
  and $x$ is the remaining string. Assume the result for $x$ and arbitrary
  states. Then, using Definition~1.11iii for the first and last equalities,
  \begin{align*}
  \overline{\delta^\cap}(\langle s,t\rangle,\pmb{a}x)
  &=\overline{\delta^\cap}(\delta^\cap(\langle s,t\rangle,\pmb{a}),x) \\
  &=\overline{\delta^\cap}(\langle\delta_1(s,\pmb{a}),\delta_2(t,\pmb{a})\rangle,x) \\
  &=\langle\DBAR_1(\delta_1(s,\pmb{a}),x),\DBAR_2(\delta_2(t,\pmb{a}),x)\rangle \\
  &=\langle\DBAR_1(s,\pmb{a}x),\DBAR_2(t,\pmb{a}x)\rangle. \qed
  \end{align*}
  \end{proof}
...
\item \begin{enumerate}[(a)]
  ...
  word was deleted immediately before it and moves into the $1$-copy.  From that
  moment on, if the original machine would have been in state $s$ after reading
  the undeleted word, the new machine is in the state $\langle s,1\rangle$ after
  reading the shortened word. Thus the $1$-copy simulates the remainder of the
  computation of $A$ on the undeleted word. The extra final states in the
  $0$-copy handle the case where the deleted letter was the last letter of the
  original word. Hence this NDFA accepts exactly $Y(L)$.
\end{enumerate}
...
\]
To see what $A'$ accepts, read an input word $x=x_1x_2\cdots x_k$. In state
$\langle s,0\rangle$ the machine is about to read an odd-positioned letter of
the original word, so it reads one actual input symbol and moves to
$\langle\delta(s,x_i),1\rangle$. In state $\langle s,1\rangle$ it is about to
skip an even-positioned letter of the original word, so it guesses that deleted
letter by a $\lambda$-move to $\langle\delta(s,c),0\rangle$ for some $c\in\Sigma$.
Thus every other letter is guessed and deleted, and the machine accepts
precisely the words obtained from words of $L$ by deleting the even-positioned
letters.
...
\begin{enumerate}[(a)]
  ...
  \]
  We show $P\notin\DSIG$ by the pumping lemma.  Assume $P$ were regular, and
  ...
  $uv^{q+1}w\notin P$, contradicting the pumping lemma.  Therefore
  $P\notin\DSIG$.

  \item \textbf{$F_\Sigma$ closed under $I$.}  $I(L)=L\cap P\subseteq L$; if $L$ is finite, so is $I(L)$.

  \item \textbf{$C_\Sigma$ closed under $I$? No.}  Take $L=\Sigma^*$.  Then $L\in C_\Sigma$, but $I(L)=P$, the set of prime-length words.  $P$ is neither finite nor cofinite, since there are infinitely many primes and infinitely many non-primes.  \textbf{Not closed.}
...
\item \begin{enumerate}[(a)]
  ...
  \begin{proof}
  We prove the stronger statement, for all states $s$, by induction on $|x|$.

  \emph{Base:} $\overline{\delta^\psi}(s,\lambda)=s=\DBAR(s,\lambda)=\DBAR(s,\PBAR(\lambda))$ (since $\psi(\lambda)=\lambda$).

  \emph{Step:} Let the string be $\pmb{b}x$, where $\pmb{b}$ is a single letter
  and $x$ is the remaining string. Assume the result for $x$ and arbitrary
  states. Then, using Definition~1.11iii for the first and last equalities,
  \[
  \overline{\delta^\psi}(s,\pmb{b}x)
  =\overline{\delta^\psi}(\delta^\psi(s,\pmb{b}),x)
  =\overline{\delta^\psi}(\DBAR(s,\psi(\pmb{b})),x)
  =\DBAR(\DBAR(s,\psi(\pmb{b})),\PBAR(x))
  =\DBAR(s,\psi(\pmb{b})\PBAR(x))
  =\DBAR(s,\PBAR(\pmb{b}x)).
  \]
  This is exactly the desired statement for $\pmb{b}x$. \qed
  \end{proof}
