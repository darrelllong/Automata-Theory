...
injective, so if $\overline{\psi_3}(L)$ were FAD then
$L=\overline{\psi_3^{-1}}(\overline{\psi_3}(L))$ would be FAD by Theorem~5.10.
\end{enumerate}

%% 5.69
\item Over $\Sigma=\{\pmb{a}\}$ there is only one homomorphism under which
\NSIG\ is obviously not closed, namely the erasing homomorphism
...
Hence the fact that $M\notin\DSIG$ says nothing by itself about whether
$K=\psi(M)$ is FAD.
