...
\delta(\langle\pmb{aa}\rangle,\pmb{a})=\{\langle\pmb{a}\rangle\},\qquad
...
\]
State $S$ generates exactly the words over $\{\pmb{a},\pmb{b}\}$ and may stop
at any point by using $S\to\lambda$; there is no $\pmb{c}$-production.
\end{enumerate}
...
\]
Here $S$ generates all strings that already contain two consecutive $\pmb{b}$s.
State $A$ generates strings whose leftmost symbol is one $\pmb{b}$ away from
creating the required pair, and $B$ generates the arbitrary prefix to the left of
that pair.

\item Same number of $\pmb{a}$s as $\pmb{b}$s: no left-linear grammar exists.
By Theorem~8.2, every left-linear grammar generates a FAD language, and the
corresponding part of Exercise~8.6 is not FAD.
...
\]
Here $S$ generates strings with an even number of $\pmb{a}$s, while $A$
generates strings with an odd number of $\pmb{a}$s.  Appending $\pmb{b}$ or
$\pmb{c}$ preserves the parity, and appending $\pmb{a}$ changes it.
...
\]
Here $S$ generates exactly the target strings: either $\lambda$, or a string
whose final symbol is $\pmb{a}$ or $\pmb{c}$.  State $A$ generates the arbitrary
prefix that may precede that final non-$\pmb{b}$ symbol.
...
\]
Here $S$ generates exactly the words over $\{\pmb{a},\pmb{b}\}$ and may stop at
any point; there is no production that appends $\pmb{c}$.
\end{enumerate}
...
\item Compute the complement DFA $A^c$ (as outlined in the proof of Theorem 5.1) by keeping the same states, start state,
...
a $\lambda$-move from $i$ to one of the states in the start set $S_0$, following
the old transitions of $A$, and then taking a final $\lambda$-move into $f$.
Conversely, any accepting path in $A'$ must begin with one of the
$\lambda$-moves from $i$ to a state in $S_0$, use only old transitions while
reading the input, and end with a $\lambda$-move from an old final state into
$f$.  Hence
\[
L(A')=L(A),
\]
...
\item No. $L(C) = \{w \mid w = w^r\}$ is not regular; it requires unbounded
memory to match the first and last symbols. Thus $L(C)$ is not FAD.
...
If $A_G$ (the NDFA constructed from $G$) is disconnected, it means some states (nonterminals) are not reachable from the start state. This means the corresponding nonterminals in $G$ are \emph{useless}: they cannot appear in any derivation from the start symbol. The portions of $G$ defining those nonterminals contribute nothing to $L(G)$ and can be removed (along with all productions involving the useless nonterminals) without affecting the language generated.
...
define
\[
G_A=\langle S\cup\{Z\},\Sigma,Z,P\rangle
\]
where $Z\notin S$, by adding
\begin{itemize}
\item a production $Z\to s$ for every start state $s\in S_0$,
...
\item \textbf{Proof}: The productions from $Z$ choose exactly one initial state
from the NDFA start set $S_0$.  Every later production records one NDFA move:
letter transitions
...
Conversely, any nonempty word satisfying this count condition can be split
around two occurrences of $\pmb{a}$ and one occurrence of $\pmb{b}$ so that the
four intervening pieces satisfy the same count condition; induction on the
length of the word then gives a derivation from $S$.
...
\]
The only way to begin a derivation for a string longer than $\pmb{b}$ is to
choose $S\to LR$.  After $L\to LD$ has been used $j-1$ times and $L\to D$ has
removed the $L$, the sentential form is $D^jR$.  In any terminal derivation,
each of the first $j-1$ occurrences of $D$ must eventually meet $R$ and use
$DR\to\pmb{b}R$, while the last occurrence must use $DR\to\pmb{bb}$; using
$DR\to\pmb{bb}$ earlier leaves a $D$ with no possible terminal completion.
Each earlier $D$ crosses the current block of $\pmb{b}$'s by
$D\pmb{b}\to\pmb{bb}D$, thereby doubling that block before passing control to
the next $D$.  Thus $D^jR$ yields exactly $\pmb{b}^{2^j}$, and every
$j\geq 1$ is obtained by the corresponding number of $L\to LD$ steps.  The
separate rule $S\to\pmb{b}$ gives the $j=0$ case, so the grammar generates all
and only the strings in $\Phi$.
\end{enumerate}
...
\[
G_3: S \to \pmb{a}S\pmb{c} \mid \pmb{c}S\pmb{a}
      \mid \pmb{b}S\pmb{c} \mid \pmb{c}S\pmb{b}
      \mid SS \mid \lambda
\]
Every production preserves the difference
$|x|_{\pmb{c}}-(|x|_{\pmb{a}}+|x|_{\pmb{b}})$: the four wrapping productions add
one $\pmb{c}$ and one non-$\pmb{c}$ symbol, $SS$ concatenates two already
balanced strings, and $S\to\lambda$ supplies the empty balanced string.  Hence
the grammar generates only strings in $L_3$.

Conversely, let $x\in L_3$.  If a proper nonempty prefix of $x$ is already
balanced, split $x=uv$ at the first such prefix and derive $u$ and $v$ by
induction, then use $S\to SS$.  If no proper prefix is balanced, the first and
last symbols must be one $\pmb{c}$ and one symbol from
$\{\pmb{a},\pmb{b}\}$; removing those two symbols leaves a shorter balanced
string, which is handled by one of
$S\to \pmb{a}S\pmb{c}$, $S\to \pmb{b}S\pmb{c}$,
$S\to \pmb{c}S\pmb{a}$, or $S\to \pmb{c}S\pmb{b}$.  Thus all required strings
are generated.  For example,
$S\Rightarrow \pmb{c}S\pmb{b}\Rightarrow \pmb{cc}S\pmb{ab}
\Rightarrow \pmb{ccab}$.
\end{enumerate}
