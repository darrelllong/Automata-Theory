...
\item \textbf{Fix}: Replace it by the genuine production
\[
Z \to S_1 S_2.
\]
If $\Omega_1$ and $\Omega_2$ are not disjoint, first rename nonterminals so
that they are.  Then
\[
G^*=\langle \Omega_1\cup\Omega_2\cup\{Z\},\Sigma,Z,P_1\cup P_2\cup\{Z\to S_1S_2\}\rangle
\]
is a valid context-free grammar.

\item \textbf{Proof}: If $w\in L(G^*)$, then the first step must be
$Z\Rightarrow S_1S_2$.  The symbols from $S_1$ can then derive some
$u\in L(G_1)$, and the symbols from $S_2$ can derive some $v\in L(G_2)$, so
$w=uv\in L(G_1)\cdot L(G_2)$.  Conversely, if $w=uv$ with
$u\in L(G_1)$ and $v\in L(G_2)$, then
$Z\Rightarrow S_1S_2 \DRightarrow uv = w$.  Hence
$L(G^*)=L(G_1)\cdot L(G_2)$. \qed
\end{enumerate}
...
\textbf{Remark}: As written, the displayed productions $X_B\to\gamma\beta_i\alpha$
cannot be the intended replacement if $X_B$ is a new nonterminal, because
nothing in the grammar would force those rules to be used.  The natural
generalization of Lemma~9.1 is to replace
\[
X\to\gamma B\alpha
\]
by the family
...
\]
With that interpretation, the proof is exactly the same substitution argument
used in Lemma~9.1.

\textbf{Proof}: Any derivation in $G$ that uses
