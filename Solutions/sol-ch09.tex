...
In a left-linear grammar, every production has the form $A \to w$ or
$A \to Bw$, where $B$ is a nonterminal and $w \in \Sigma^*$.  Thus the parse
trees are the mirror image of the right-linear trees discussed in Example~9.2.
More precisely:
\begin{itemize}
\item Whenever a node is expanded by a production of the form $A \to Bw$, its
leftmost child is the only child labeled by a nonterminal; all remaining
children are terminals.
\item Following these leftmost nonterminal children produces a single
``left spine'' of the tree.  Every nonterminal in the parse tree lies on this
spine.
\item The last nonterminal on the spine is expanded by a rule $A \to w$, so the
bottom of the tree consists entirely of terminal leaves.
\end{itemize}
So left-linear parse trees are maximally left-branching, just as right-linear
parse trees are maximally right-branching.
...
\]
Here $A$ generates the balanced $\pmb{a}^n\pmb{b}^n$ block and $B$
generates the independent balanced $\pmb{c}^m\pmb{d}^m$ block. The start
symbol concatenates these two independently generated pieces.

\item $\{\pmb{a}^i\pmb{b}^j\pmb{c}^j\pmb{d}^i \mid i,j \in \NN\}$:
\[
S \to \pmb{a}S\pmb{d} \mid T \mid \lambda, \quad T \to \pmb{b}T\pmb{c} \mid \lambda.
\]
Here $S$ accounts for the outer matching $\pmb{a}/\pmb{d}$ symbols, and
then hands control to $T$ for the middle matching $\pmb{b}/\pmb{c}$ symbols.
The rule $S\to\lambda$ is the case $i=j=0$.

\item $\{\pmb{a}^n\pmb{b}^n\pmb{c}^m\pmb{d}^m\} \cup \{\pmb{a}^i\pmb{b}^j\pmb{c}^j\pmb{d}^i\}$:
\[
S \to S_1 \mid S_2
\]
where $S_1$ generates the first language (as in part (a)) and $S_2$
generates the second (as in part (b)). The two productions from $S$ are the
grammar-level union construction.
\end{enumerate}
...
\begin{enumerate}
\item \textbf{What is wrong}: The symbol $\cdot$ in $Z \to S_1 \cdot S_2$ is
meta-notation for concatenation, not a symbol of the grammar.  A context-free
production must have a right side in $(\Omega \cup \Sigma)^*$, so the proposed
rule is not literally a valid production.

\item \textbf{Fix}: Replace the invalid production with a construction named for
concatenation.  If $\Omega_1$ and $\Omega_2$ are not disjoint, first rename
nonterminals so that they are.  When $G_1$ and $G_2$ are pure context-free
grammars, define
\[
G^{\cdot}=\langle \Omega_1\cup\Omega_2\cup\{C\},\Sigma,C,
P_1\cup P_2\cup\{C\to S_1S_2\}\rangle,
\]
where $C\notin\Omega_1\cup\Omega_2\cup\Sigma$.
This is again pure, since the added right side is nonempty.  If either original
grammar is non-pure only because it uses the special start rules
$Z_i\to\lambda$ and $Z_i\to S_i$, use the corresponding pure core and add
$C\to S_i$ when the other language contains $\lambda$.  Finally, add the
special rules $Z\to\lambda$ and $Z\to C$ only when both languages contain
$\lambda$.  The simpler single production $C\to S_1S_2$ is already enough for the
pure case and fulfills the exercise's requested repair.

\item \textbf{Proof}: In the pure case, if $w\in L(G^{\cdot})$, then the first
step must be $C\Rightarrow S_1S_2$.  The symbols from $S_1$ can then derive
some $u\in L(G_1)$, and the symbols from $S_2$ can derive some $v\in L(G_2)$,
so $w=uv\in L(G_1)\cdot L(G_2)$.  Conversely, if $w=uv$ with
$u\in L(G_1)$ and $v\in L(G_2)$, then
$C\Rightarrow S_1S_2 \DRightarrow uv = w$.  Thus
$L(G^{\cdot})=L(G_1)\cdot L(G_2)$.

The non-pure cases add only the productions needed to represent concatenations
where one factor is $\lambda$; the special start rules are used exactly when
both factors may be $\lambda$.  Therefore the resulting grammar is a valid
context-free grammar for $L(G_1)\cdot L(G_2)$. \qed
\end{enumerate}
...
\[
S \to R \mid R\pmb{\cdot}R \mid R\pmb{\cup}R
\]
where $R$ is the original grammar from Example~9.3:
\[
R \to \pmb{a}\mid\pmb{b}\mid\pmb{c}\mid\pmb{\epsilon}\mid\pmb{\emptyset}\mid
\pmb{(}R\pmb{\cdot}R\pmb{)}\mid\pmb{(}R\pmb{\cup}R\pmb{)}\mid R^{\pmb{*}}.
\]
The new start symbol $S$ therefore generates all old words, and in addition it
allows the outermost parentheses of a binary expression to be omitted.

\item Grammar allowing extraneous parentheses:
\[
A \to \pmb{(}A\pmb{)} \mid \pmb{a}\mid\pmb{b}\mid\pmb{c}\mid\pmb{\epsilon}\mid
\pmb{\emptyset}\mid \pmb{(}A\pmb{\cdot}A\pmb{)}\mid
\pmb{(}A\pmb{\cup}A\pmb{)}\mid A^{\pmb{*}}.
\]
The rule $A \to \pmb{(}A\pmb{)}$ allows any expression to be wrapped in an
extra pair of parentheses, and repeated use of that rule gives arbitrarily many
extraneous pairs.
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Lower bound}: Let $|x|=n>0$.  Ignoring the optional initial
rule $Z\to S$, the parse tree below $S$ is a binary tree with exactly $n$
terminal leaves, because every nonterminal production in CNF has the form
$A\to BC$ or $A\to \pmb{d}$.  A binary tree of depth $h$ has at most $2^h$
leaves, so $2^h\geq n$ and therefore
\[
h\geq \lceil \log_2 n\rceil.
\]
If one also counts the initial edge $Z\to S$, this lower bound increases by~1.

\item \textbf{Upper bound}: The deepest possible binary parse tree with $n$
terminal leaves is the maximally skewed one, where each internal node has one
terminal child and one child that continues the derivation.  Such a tree has
depth $n-1$ below $S$.  Including the optional initial rule $Z\to S$, the full
CNF parse tree has depth at most $n$.

\textbf{Summary}: With the full CNF tree rooted at $Z$,
\[
\lceil \log_2 |x| \rceil + 1 \leq \text{depth} \leq |x|.
\]
If the initial $Z\to S$ edge is ignored, the bounds become
$\lceil \log_2 |x| \rceil \leq \text{depth} \leq |x|-1$.
\end{enumerate}
...
$R \to (R\pmb{\cup}R)$: Break as $R \to T_{(}D_4$, $D_4 \to RD_5$, $D_5 \to T_\cup D_6$, $D_6 \to RT_{)}$.
...
Applying the substitutions from Theorem~9.6 gives the following equivalent GNF
grammar (with one new nonterminal $S_3$):
\[
\begin{aligned}
S_1&\to \pmb{c}S_1\pmb{e}\mid \pmb{c}\pmb{b}\mid \pmb{c}S_1\pmb{e}S_3\mid \pmb{c}\pmb{b}S_3,\\
S_2&\to \pmb{c}\mid \pmb{c}S_1\pmb{e}S_2\mid \pmb{c}\pmb{b}S_2\mid
\pmb{c}S_1\pmb{e}S_3S_2\mid \pmb{c}\pmb{b}S_3S_2,\\
S_3&\to \pmb{c}S_1\pmb{e}\mid \pmb{c}\pmb{b}\mid \pmb{c}S_1\pmb{e}S_3\mid
\pmb{c}\pmb{b}S_3\\
&\qquad\mid \pmb{c}S_1\pmb{e}S_2S_1\pmb{e}\mid \pmb{c}\pmb{b}S_2S_1\pmb{e}\mid
\pmb{c}S_1\pmb{e}S_3S_2S_1\pmb{e}\mid \pmb{c}\pmb{b}S_3S_2S_1\pmb{e}\\
&\qquad\mid \pmb{c}S_1\pmb{e}S_2\pmb{b}\mid \pmb{c}\pmb{b}S_2\pmb{b}\mid
\pmb{c}S_1\pmb{e}S_3S_2\pmb{b}\mid \pmb{c}\pmb{b}S_3S_2\pmb{b}\\
&\qquad\mid \pmb{c}S_1\pmb{e}S_2S_1\pmb{e}S_3\mid
\pmb{c}\pmb{b}S_2S_1\pmb{e}S_3\mid
\pmb{c}S_1\pmb{e}S_3S_2S_1\pmb{e}S_3\mid
\pmb{c}\pmb{b}S_3S_2S_1\pmb{e}S_3\\
&\qquad\mid \pmb{c}S_1\pmb{e}S_2\pmb{b}S_3\mid
\pmb{c}\pmb{b}S_2\pmb{b}S_3\mid
\pmb{c}S_1\pmb{e}S_3S_2\pmb{b}S_3\mid
\pmb{c}\pmb{b}S_3S_2\pmb{b}S_3.
\end{aligned}
\]
Every production now begins with the terminal $\pmb{c}$, so the grammar is in
GNF.
...
\textbf{Proof}: The generalized replacement is to replace
\[
X\to\gamma B\alpha
\]
by the family
\[
X\to\gamma\beta_1\alpha,\ \ldots,\ X\to\gamma\beta_m\alpha.
\]
Any derivation in $G$ that uses
$X\to\gamma B\alpha$ and then expands $B$ by some rule $B\to\beta_i$ can be
replaced by the single step
$X\to\gamma\beta_i\alpha$.  All other productions are unchanged, so every word
of $L(G)$ still belongs to $L(G')$.

Conversely, each new production $X\to\gamma\beta_i\alpha$ in $G'$ can be
simulated in $G$ by first using $X\to\gamma B\alpha$ and then
$B\to\beta_i$.  Hence the two grammars generate the same language. \qed
...
Assume $\Gamma$ is context free with pumping constant $n$. Choose $s = \pmb{0}^n\pmb{1}^n\pmb{2}\pmb{0}^n\pmb{1}^n \in \Gamma$.  Then $|s|=4n+1$.
...
Assume $\Psi$ is context free with pumping constant $n$. Choose $s = \pmb{0}^n\pmb{1}^n\pmb{0}^n\pmb{1}^n \in \Psi$. $|s| = 4n$.
...
$uv^2wx^2y\notin\Phi'$, contradicting Ogden's lemma.  Therefore
...
\begin{enumerate}
\item Let $d(v)$ denote the number of distinguished positions in the subtree
rooted at a node $v$.  Define the path recursively as follows.
\begin{itemize}
\item \textbf{Boundary}: If $v$ is a leaf, stop.
\item \textbf{Rule}: If $v$ has two children $v_\ell$ and $v_r$, choose the
child with the larger value of $d$.  If $d(v_\ell)=d(v_r)$, choose either one.
\end{itemize}
Starting at the root and repeating this rule yields a path ending at a
distinguished leaf.

\item Because the chosen child always contains at least half of the
distinguished positions of its parent, a node that is $k$ levels above the end
of the path still has at least $2^k$ distinguished positions in its subtree.
Now choose the \emph{highest} node $N$ on the path whose subtree contains no
more than $n$ distinguished positions.  Such a node exists because the final
leaf contains exactly one distinguished position.  Its parent (if any) contains
more than $n$ distinguished positions, so $N$ must still contain more than
$n/2 = 2^{|\Omega|}$ distinguished positions.  Hence the portion of the path
inside the subtree rooted at $N$ still has more than $|\Omega|$ interior nodes,
and some nonterminal label must repeat there.  Let the higher and lower
occurrences of that repeated label be $R$.

If the higher $R$ derives $vRx$, and the lower $R$ derives $w$, then
\[
S\DRightarrow uRy \DRightarrow uvRxy \DRightarrow uvwxy.
\]
The subtree rooted at the lower $R$ contains a distinguished position, so $w$
contains one.  The higher $R$ lies inside the subtree rooted at $N$, so $vwx$
contains no more than $n$ distinguished positions.  And since the two
occurrences of $R$ are distinct, at least one distinguished position lies in
the part of the higher subtree outside the lower one, so $vx$ contains a
distinguished position.  Replacing the lower $R$-subtree
by repeated copies of the higher one now gives the pumped strings
$uv^iwx^iy$, exactly as required in Theorem~9.9. \qed
\end{enumerate}
...
\item \textbf{Intersection}: Not closed.  Let
\[
A=\{\pmb{a}^{2^j}\mid j\in\NN\}
\qquad\text{and}\qquad
B=\pmb{a}^*-A.
\]
As in part (a), both $A$ and $B$ belong to \XSIG, but
\[
A\cap B=\emptyset,
\]
which is context free.  Hence \XSIG\ is not closed under intersection.

\item \textbf{Kleene closure}: Not closed.  Let
\[
A=\{\pmb{a}^{2^j}\mid j\in\NN\}.
\]
Then $A\in\XSIG$.  Since
...
that $A$ is regular, contradiction.  So $A'\in\XSIG$.

If $B'$ were context free, then Theorem~9.15 would make $B'$ regular. Removing
the single word $\lambda$ would then show that
$B=B'-\{\lambda\}$ is regular, contradiction. Hence $B'\in\XSIG$.

Now every word $\pmb{a}^n$ belongs to $A'B'$: if $\pmb{a}^n\in A$, then
$\pmb{a}^n=\pmb{a}^n\lambda$ with $\pmb{a}^n\in A'$ and $\lambda\in B'$; if
$\pmb{a}^n\notin A$, then $\pmb{a}^n\in B$ and
...
Conversely, suppose $x^r\in L(G^r)$. Applying the forward implication to the
reversed grammar $G^r$, and using $(G^r)^r=G$, gives $x\in L(G)$. Taking
...
%%------------------------------------------------------------
\item \textbf{Determine if $\CSIG$ is closed under operator $T$ ($T(L) = \{w^r w \mid w \in L\}$).}
...
\]
We claim $T(L)\notin\CSIG$.  Suppose instead that $T(L)$ is context free, and
...
pumped word changes only that rightmost block.  The leftmost $\pmb{a}$-block
remains of length $N$, so the pumped word cannot have the required form
$\pmb{a}^n\pmb{b}\pmb{a}^{2n}\pmb{b}\pmb{a}^n$, in which the two outer
$\pmb{a}$-blocks have the same length.
...
\textbf{Disproof}: Let $\Sigma=\{\pmb{a},\pmb{b}\}$ and let
\[
K=\{\pmb{a}^n\pmb{b}^n\pmb{a}^n\mid n\in\NN\}.
\]
By the pumping-lemma argument used in the next exercise, $K$ is not context
free.  Its complement $\Sigma^*-K$ is context free: it is the union of the
regular language $\Sigma^*-\pmb{a}^*\pmb{b}^*\pmb{a}^*$ with the strings
$\pmb{a}^i\pmb{b}^j\pmb{a}^k$ for which $i\neq j$ or $j\neq k$, and those
inequality cases are context free.

Now take
\[
L_1=\Sigma^*,\qquad L_2=\Sigma^*-K.
\]
Both $L_1$ and $L_2$ are context free, but
\[
L_1-L_2=K,
\]
which is not context free.  Hence
$\mathfrak{C}_{\{\pmb{a},\pmb{b}\}}$ is not closed under relative complement.

The same conclusion also follows from the closure properties: if
$\mathfrak{C}_{\{\pmb{a},\pmb{b}\}}$ were closed under relative complement and
we know it is closed under union, then it would be
closed under complement (since $\overline{L_1} = \Sigma^* - L_1$, and
$\Sigma^*$ is CFL). But part (a) of the next exercise shows that
$\mathfrak{C}_{\{\pmb{a},\pmb{b}\}}$ is not closed under complement. Hence
$\mathfrak{C}_{\{\pmb{a},\pmb{b}\}}$ is \emph{not} closed under relative
complement. \qed
...
\]
Both are context free.  For example, $L_1$ is generated by
$S\to AB$, $A\to\pmb{a}A\mid\lambda$,
$B\to\pmb{b}B\pmb{a}\mid\lambda$, and $L_2$ is generated by
$S\to CD$, $C\to\pmb{a}C\pmb{b}\mid\lambda$,
$D\to\pmb{a}D\mid\lambda$.  Their intersection is
\[
L_1 \cap L_2 = \{\pmb{a}^n\pmb{b}^n\pmb{a}^n \mid n \in \NN\},
\]
which is not context free (by the pumping lemma). So
$\mathfrak{C}_{\{\pmb{a},\pmb{b}\}}$ is not closed under intersection.

If $\mathfrak{C}_{\{\pmb{a},\pmb{b}\}}$ were closed under complement, then
$\overline{L_1}$ and $\overline{L_2}$ would be context free, and
$\overline{\overline{L_1} \cup \overline{L_2}} = L_1 \cap L_2$ would be context
free (by De Morgan and closure under union and complement). But
$L_1 \cap L_2$ is not CFL. Contradiction. So
$\mathfrak{C}_{\{\pmb{a},\pmb{b}\}}$ is not closed under complement.

\item For $|\Sigma| > 1$, choose two distinct symbols $\alpha,\beta\in\Sigma$.
Let $h:\{\pmb{a},\pmb{b}\}^*\to\Sigma^*$ be the homomorphism with
$h(\pmb{a})=\alpha$ and $h(\pmb{b})=\beta$.  Applying $h$ to the two languages
from part (a) gives context-free languages over $\Sigma$:
\[
h(L_1)=\{\alpha^i\beta^j\alpha^j\mid i,j\in\NN\},\qquad
h(L_2)=\{\alpha^i\beta^i\alpha^j\mid i,j\in\NN\}.
\]
But their intersection is
\[
h(L_1)\cap h(L_2)=\{\alpha^n\beta^n\alpha^n\mid n\in\NN\},
\]
which is not context free by the same pumping-lemma argument.  Therefore
...
%%------------------------------------------------------------
\item \textbf{Automaton-based method for Theorem 9.3.}

In Theorem~9.3, each nonterminal $B$ gives an NDFA $A^B$, and all of these
machines share the same transition relation; only the start state changes.
Instead of examining them one at a time, we can combine them into one automaton
whose states are subsets of $\Omega\cup\{\omega\}$.

Define the DFA
\[
D=\langle \{\pmb{1}\},\wp(\Omega\cup\{\omega\}),\{S\},\Delta,F\rangle,
\]
where
\[
\Delta(T,\pmb{1})=\bigcup_{A\in T}\delta(A,\pmb{1}),
\]
and $F$ is the collection of subset states that contain $\omega$.

The meaning of a subset state $T$ is: after some number of grammar-expansion
steps, the nonterminals in $T$ are exactly the ones that can occur in the
corresponding automaton search.  Then:
\begin{itemize}
\item Reachability from the start state $\{S\}$ identifies the nonterminals
that are accessible from $S$.
\item A nonterminal $B$ can derive a terminal string exactly when some state in
$D$ reachable from $\{B\}$ lies in $F$, that is, contains $\omega$.
\item After removing nonterminals that fail one of those tests, the same
reachability check detects the remaining disconnected items.
\end{itemize}
So the subset-labeled automaton provides the same information as the family of
machines $A^B$, but packages the computation into one finite automaton over
$\wp(\Omega\cup\{\omega\})$.
...
\begin{enumerate}
\item Such grammars are \emph{not} a normal form for all context-free
languages.  If a grammar is in GNF and no right side has more than two symbols,
then every production has the form
\[
A\to \pmb{a}\qquad\text{or}\qquad A\to \pmb{a}B.
\]
That is exactly the form of a right-linear grammar.  Therefore every language
generated by such a grammar is regular.  But
$\{\pmb{a}^n\pmb{b}^n\mid n\in\NN\}$ is context free and not regular, so it
cannot be generated by any grammar of this restricted type.

\item By the same observation, the grammars that do satisfy this restriction are
precisely the right-linear grammars.  Hence they generate exactly the regular
languages. \qed
\end{enumerate}

%%------------------------------------------------------------
\item \textbf{Determine if $L^*$ must be regular for \emph{any}
collection $L$ over $\Sigma$.}
...
$A$ generates $\{\pmb{a}^n\pmb{b}^n\mid n\in\NN\}$ and $B$ generates
$\pmb{c}^*$. So
$L(G) = \{\pmb{a}^n\pmb{b}^n\pmb{c}^m\mid n,m\in\NN\}$. \qed
