...
\textbf{Algorithm}:
\begin{enumerate}
\item Use the usual $\lambda$-NDFA form of the structural construction in
Lemma~6.2.  The resulting regular-expression-to-automaton correspondence is
recorded in Corollary~6.1 and summarized by Kleene's Theorem~6.5. Give every
subexpression a fresh entry state
and a fresh exit state; the union, concatenation, and star cases join these
pairs with $\lambda$-moves.  The exit state for the complete expression is the
only final state.
\item Thus every regular expression has a $\lambda$-NDFA with exactly one
final state that accepts its language.  The answer is always Yes. \qed
\end{enumerate}
...
xy\in L.
\]
So whenever $\lambda\in L$, we must have $L\cdot L\subseteq L$.
...
\textbf{Sufficiency when $\lambda\in L$ and $L\cdot L\subseteq L$}:
...
Repeated application of $L\cdot L\subseteq L$ shows that the whole input word
also belongs to $L$. So this new machine again accepts exactly $L$ and has one
final state.
...
\]
\end{enumerate}
This inclusion is exactly the required condition, so the question is decidable.
\qed
...
\begin{enumerate}
\item Convert $R_1, R_2$ to DFAs $A_1, A_2$.
\item Apply the DFA equivalence algorithm of Theorem~12.3 to $A_1$ and $A_2$.
That algorithm constructs the composite DFA and computes its state-equivalence
relation, then compares the two start states.
\item Answer yes exactly when the two start states are state-equivalent in that
composite DFA.  This is equivalent to $L(A_1)=L(A_2)$, and hence to
$L(R_1)=L(R_2)$. \qed
\end{enumerate}
...
\]
For each $s\in S_1$ and $t\in S_2$, form the two DFAs
\[
B_{1,s}=\langle\Sigma,S_1,s_1,\delta_1,\{s\}\rangle,
\qquad
B_{2,t}=\langle\Sigma,S_2,s_2,\delta_2,\{t\}\rangle.
\]
Each has exactly one final state, and
\[
L(B_{1,s})=\{x\mid\DBAR_1(s_1,x)=s\},
\qquad
L(B_{2,t})=\{x\mid\DBAR_2(s_2,x)=t\}.
\]
Because the minimal machines are connected, these languages are precisely the
equivalence classes of $R^{M_1}$ and $R^{M_2}$, respectively. Apply
Theorem~12.3 to every pair $B_{1,s},B_{2,t}$. Answer Yes exactly when
every class found from $M_1$ is equal to exactly one class found from $M_2$, and
conversely. This is a finite collection of applications of Theorem~12.3, so it
decides whether $R_{L_1}=R_{L_2}$. \qed
...
returned union is the desired set. At stage $i$, each state in $T_i$ requires
$\|\Sigma\|$ transition-table lookups, one for each input letter; since
$T_i\subseteq S$, there are at most $n\|\Sigma\|$ lookups at that stage. The
procedure halts because it performs only $2n-1$ finite computations. \qed
...
Let
\[
M_i=\langle\Sigma,\Gamma,S_i,s_{0_i},\delta_i,\omega_i\rangle
\qquad(i=1,2).
\]
Define the directed product graph to have vertex set $S_1\times S_2$, start
vertex $(s_{0_1},s_{0_2})$, and, for each $a\in\Sigma$, an $a$-labeled edge
\[
...
\textbf{Algorithm}:
\begin{enumerate}
\item Perform breadth-first search from $(s_{0_1},s_{0_2})$, examining each
reachable pair $(q_1,q_2)$ once.
\item For each such pair and each $a\in\Sigma$, compare $\omega_1(q_1,a)$ and
$\omega_2(q_2,a)$. If they differ, answer No.
\item Otherwise, place the endpoint
$(\delta_1(q_1,a),\delta_2(q_2,a))$ of the defined $a$-edge on the search queue
unless it was already examined.
\end{enumerate}
Because the defined product graph has $\|S_1\|\cdot\|S_2\|$ vertices, this
search halts. It answers Yes exactly when every pair reached by the same input
word produces the same next output symbol on every possible next input, which
...
\textbf{Proof}: $Z_0\subseteq Z_1$ since $Z_1 = Z_0\cup(\cdots) \supseteq Z_0$. By induction: if $Z_k\subseteq Z_{k+1}$, then $Z_{k+1}\subseteq Z_{k+2}$ since any nonterminal in $Z_{k+1}$ is reachable in at most $k+1$ steps, hence is reachable in at most $k+2$ steps. The chain is bounded above by $\Omega$ (which is finite). \qed
...
Let $M$ be the given Mealy machine and $A$ the given Moore machine. By
Definition~7.19, construct the corresponding Mealy machine $M_A$ from $A$.
Theorem~7.7 gives
\[
f_{M_A}=f_A,
\]
where the translation functions are those of Definition~7.18.
Therefore
\[
M\text{ is equivalent to }A
\quad\Longleftrightarrow\quad
M\text{ is equivalent to }M_A.
\]

\textbf{Algorithm}:
\begin{enumerate}
\item Construct $M_A$ using Definition~7.19.
\item Apply the product-graph procedure from Exercise~12.19 to $M$ and $M_A$.
\end{enumerate}
The construction and the search are finite, so this decides the question.
\qed
...
\begin{enumerate}
\item \textbf{Method}: BFS/DFS directly on the NDFA $A$ (viewed as a graph).
Start from the start set $S_0$, compute its $\lambda$-closure, then follow
transitions. $L(A)=\emptyset$ iff no final state is reachable from any state in
$S_0$ (via any sequence of transitions and $\lambda$-moves). Reachability on the
finite graph of $A$ is enough; there is no need to construct the exponential
subset DFA.
...
deeper than the earlier decidability arguments in the chapter. The standard
regularity algorithm for DPDAs can be outlined as follows.
\begin{enumerate}
\item Put $M$ into an equivalent normal form in which the effects of entering
and later leaving a stack symbol can be summarized by a finite collection of
control-state pairs.
\item Compute these finite summaries for all reachable stack symbols and use
the DPDA equivalence procedure to identify summaries with the same residual
input language.
\item Test the recurrent stack-growth patterns. If they yield infinitely many
pairwise distinct residual languages, then $L(M)$ is not regular. Otherwise the
finitely many residual languages are the states of an equivalent DFA, which the
procedure constructs.
\end{enumerate}
This procedure always terminates because the normal-form summaries are finite
descriptions and the equivalence tests used in the recurring cases are
decidable. It answers Yes exactly when the constructed finite collection of
residual languages recognizes $L(M)$.
...
\item \textbf{Context-free grammars}: Undecidable. Use the standard reduction
from the Post correspondence problem. From an instance $P$, construct a
context-free grammar $G_P$ that generates every well-formed candidate encoding
except an encoding of a solution of $P$; malformed encodings and candidates
with a first mismatch are generated directly. Thus
\[
L(G_P)=\Sigma_P^*
\quad\Longleftrightarrow\quad
P\text{ has no solution}.
\]
The construction of $G_P$ is effective, so a universality procedure for
context-free grammars would decide the Post correspondence problem. Hence no
such procedure exists.
\item \textbf{Pushdown automata}: Undecidable. Given a context-free grammar
$G$, Theorem~10.2 effectively constructs a pushdown automaton $P_G$
with $\Lambda(P_G)=L(G)$. Therefore a universality procedure for pushdown
automata would decide the context-free-grammar case just proved. \qed
\end{enumerate}
...
\item \textbf{All equivalence classes are finite}: No. By Definition~1.1,
$\Sigma$ is nonempty, so $\Sigma^*$ is infinite. The relation $R_{L(A)}$ has
only finitely many equivalence classes because $L(A)$ is FAD (equivalently,
the reduced connected DFA is isomorphic to $A_{R_{L(A)}}$ by
Theorem~3.1). If all of those finitely many classes were finite, then
their union $\Sigma^*$ would be finite, a contradiction. Thus the one-step
decision procedure always answers No. \qed
\end{enumerate}
