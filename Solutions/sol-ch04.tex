...
\item Figure~4.25(c): start state $\{r_0\}$, final states $\{r_0\}$ and
$\{r_0,r_1\}$.
\[
\begin{array}{c|ccc}
 & \pmb{a} & \pmb{b} & \pmb{c}\\\hline
...
\item Figure~4.25(e): start state $\{s_1\}$, final state $\{s_1,s_2\}$.
\[
\begin{array}{c|ccc}
 & \pmb{a} & \pmb{b} & \pmb{c}\\\hline
...
\[
E=\langle\{\pmb{a},\pmb{b}\},\{s_1,s_2,s_3\},\{s_1\},\delta_{\lambda},\{s_2\}\rangle
\]
with ordinary transitions
\[
\delta_{\lambda}(s_1,\pmb{a})=\{s_1,s_2\},\qquad
\delta_{\lambda}(s_1,\pmb{b})=\{s_3\},
\]
\[
...
\]
\[
\delta_{\lambda}(s_3,\pmb{a})=\emptyset,\qquad
\delta_{\lambda}(s_3,\pmb{b})=\{s_1\},
\]
...
\begin{enumerate}[(a)]
  ...
  \[
    ...
    \Lambda(s_2)=\{s_2\},\qquad
    \Lambda(s_3)=\{s_3\}.
  \]
  Since $\lambda\in L(E)$, Definition~4.9 adds the isolated start state $q_0$
  and also makes $q_0$ final.  Thus
  \[
  E^{\sigma}_{\lambda}=\langle\{\pmb{a},\pmb{b}\},\{q_0,s_1,s_2,s_3\},\{q_0,s_1\},
  \delta^{\sigma}_{\lambda},\{q_0,s_2\}\rangle
  \]
  with
  \[
  \delta^{\sigma}_{\lambda}(q_0,\pmb{a})=
  \delta^{\sigma}_{\lambda}(q_0,\pmb{b})=\emptyset,
  \]
  \[
  \delta^{\sigma}_{\lambda}(s_1,\pmb{a})=\{s_1,s_2\},\qquad
  \delta^{\sigma}_{\lambda}(s_1,\pmb{b})=\{s_1,s_2,s_3\},
  \]
  \[
  ...
  \]
  \[
  \delta^{\sigma}_{\lambda}(s_3,\pmb{a})=\emptyset,\qquad
  \delta^{\sigma}_{\lambda}(s_3,\pmb{b})=\{s_1,s_2\}.
  \]

  \item \emph{Convert this NDFA into a DFA (Definition~4.5).}  The full
  deterministic machine has $2^4=16$ subset states, but only three are reachable
  from the start set $\{q_0,s_1\}$:
  \[
  Q_0=\{q_0,s_1\},\qquad Q_1=\{s_1,s_2\},\qquad Q_2=\{s_1,s_2,s_3\}.
  \]
  All three are final, because $q_0\in Q_0$ and $s_2\in Q_1\cap Q_2$. Their
  transitions are
  \[
  \begin{array}{c|cc}
   & \pmb{a} & \pmb{b}\\\hline
...
\begin{enumerate}[(a)]
  ...
  \item \emph{Circuit diagram (with \textless SOS\textgreater{} and
    \textless EOS\textgreater).}  Use the same next-state equations as in
    part~(a), but OR the start pulse into the two start-state inputs:
    \[
...
\begin{enumerate}[(a)]
  ...
  \]
  The $\lambda$-moves $s_0\to s_1$ and $s_1\to s_2$ are implemented exactly as
  in Example~4.17 by OR-ing the source activity line directly into the
  destination input.  Thus the circuit can be specified by
  \[
    D_0=0,\qquad
...
\begin{enumerate}[(a)]
  ...
  \[
    ...
    \Lambda(s_2)=\{s_0,s_2\}.
  \]
  Since $\lambda\not in L(A_\lambda)$, Definition~4.9 adds $q_0$ as an extra
...
yet $\pmb{a}\not in\Sigma^*\setminus L(A)$ since $\pmb{a}\in L(A)$.  Hence the
...
\]
This is proved by the same induction as Lemma~4.2: for a single letter
$\pmb{a}$, it is just the definition
$\delta'(s,\pmb{a})=\DBAR_{\lambda}(s,\pmb{a})$, and the inductive step extends
in the usual nondeterministic manner.
...
\begin{enumerate}[(a)]
  ...
  forced under the hypothesis $\lambda\not in L$, but not uniformly in general.
\end{enumerate}
...
\begin{enumerate}[(a)]
  ...
  Example~4.15 gives the $\lambda$-free machine of Figure~4.20, so we can use
  one activity bit for each of $q_0,s_0,s_1,s_2,s_3$.  The next-state equations
  are
  \[
    q'_0=0,\qquad s'_0=0,
  \]
...
with $\lambda\not in L(A)$, construct $A''$ with exactly one start state, one
final state, and $L(A'')=L(A)$.}

\textbf{Construction.}  Let $A=\langle\Sigma,S,S_0,\delta,F\rangle$ with
$\lambda\not in L(A)$, so $S_0\cap F=\emptyset$.  Define
\[
