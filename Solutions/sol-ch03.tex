...
\textit{Step}: Let $x=ya$ with $|y|=n$. This is the tail-recursive form of
Definition~1.11iii. Using that definition at the beginning and end, together
with the homomorphism property $\mu(\delta(t,a))=\delta_{R_L}(\mu(t),a)$,
\[
\mu(\DBAR(s,ya))=\mu(\delta(\DBAR(s,y),a))=\delta_{R_L}(\mu(\DBAR(s,y)),a)=\delta_{R_L}(\DBAR_{R_L}(\mu(s),y),a)=\DBAR_{R_L}(\mu(s),ya).
\]
$\square$
...
\textit{Step}: Let $x=ya$. By the tail-recursive clause in Definition~1.11iii,
\[
...
\[
\mu(\DBAR_A(s,ya))
=\mu(\delta_A(\DBAR_A(s,y),a))
=\delta_B(\mu(\DBAR_A(s,y)),a)
=\delta_B(\DBAR_B(\mu(s),y),a)
=\DBAR_B(\mu(s),ya),
\]
where the third equality is the homomorphism property, the fourth is the induction hypothesis, and the last is again Definition 1.11iii. $\square$
