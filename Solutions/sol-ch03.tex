...
\textit{Step}: Let $x=ay$ where $a\in\Sigma$ is a single letter on the left and
$y\in\Sigma^*$ is a string of length $n$ on the right. This is the
tail-recursive form of Definition~1.11iii. Using that definition at the
beginning and end, together with the induction hypothesis for the state
$\delta(s,a)$ and the string $y$,
and with the homomorphism property $\mu(\delta(t,a))=\delta_{R_L}(\mu(t),a)$,
\[
\mu(\DBAR(s,ay))=\mu(\DBAR(\delta(s,a),y))=\DBAR_{R_L}(\mu(\delta(s,a)),y)=\DBAR_{R_L}(\delta_{R_L}(\mu(s),a),y)=\DBAR_{R_L}(\mu(s),ay).
\]
$\square$
...
\textit{Step}: Let $x=ay$ where $a\in\Sigma$ is a single letter and
$y\in\Sigma^*$ is the remaining string. By the tail-recursive clause in
Definition~1.11iii,
\[
\DBAR_{E_A}([s]_{E_A},ay)=\DBAR_{E_A}(\delta_{E_A}([s]_{E_A},a),y)=\DBAR_{E_A}([\delta(s,a)]_{E_A},y)=[\DBAR(\delta(s,a),y)]_{E_A}=[\DBAR(s,ay)]_{E_A}. \quad\square
\]
...
\textit{Step}: Clearly, if $s E_A t$, then
...
\textit{Step}: Clearly, assume the claim holds for $n$. Let $|x|\leq n+1$.
...
For every class $[s]_{E_A}$ and letter $a\in\Sigma$, the state $\delta(s,a)$ exists, so the class $[\delta(s,a)]_{E_A}$ exists as well. Thus $\delta_{E_A}([s]_{E_A},a)=[\delta(s,a)]_{E_A}$ is defined everywhere. To show it is not multiply-defined, suppose $[s]_{E_A}=[t]_{E_A}$, i.e., $s E_A t$. Then $\delta(s,a) E_A \delta(t,a)$ (proved in Exercise 3.3, $\supseteq$ direction): $L(A^{\delta(s,a)})=\{x:ax\in L(A^s)\}=\{x:ax\in L(A^t)\}=L(A^{\delta(t,a)})$. So $[\delta(s,a)]_{E_A}=[\delta(t,a)]_{E_A}$. $\square$

%%------------------------------------------------------------
\item \textbf{Show $F_{E_A}$ is well defined.}

Every $E_A$-class has at least one representative, so the description $F_{E_A}=\{[s]_{E_A}\mid s\in F\}$ addresses every class. To show membership is not multiply-defined, suppose $[s]_{E_A}=[t]_{E_A}$ and $s\in F$. Then $s E_A t\Rightarrow L(A^s)=L(A^t)\Rightarrow\lambda\in L(A^s)\Leftrightarrow\lambda\in L(A^t)\Rightarrow s\in F\Leftrightarrow t\in F$. So $t\in F$, and the definition is independent of the chosen representative. $\square$
...
\[
\mu(\DBAR_A(s,ay))
=\mu(\DBAR_A(\delta_A(s,a),y))
=\DBAR_B(\mu(\delta_A(s,a)),y)
=\DBAR_B(\delta_B(\mu(s),a),y)
=\DBAR_B(\mu(s),ay),
\]
where the third equality is the induction hypothesis, the fourth is the homomorphism property, and the last is again Definition 1.11iii. $\square$
...
\item Well-definedness: it is defined everywhere because connectedness gives, for every $s\in S$, at least one string $x_s$ with $\DBAR(s_{0A},x_s)=s$. It is not multiply-defined because if $\DBAR(s_0,x)=\DBAR(s_0,y)=s$, then $x R^A y$ (same state), and $R^A$ refines $R_{L(A)}$, so $x R_{L(A)} y$, giving $[x]_{R_{L(A)}}=[y]_{R_{L(A)}}$. $\square$
...
\item \textit{Well-defined}: this is defined everywhere on $S^c$ because, by definition of the connected part, each $s\in S^c$ has at least one string reaching it. If $x$ and $y$ both satisfy
\[
...
\end{array}
\]
The connected part of this quotient is isomorphic to the three-state machine
$A_{R_{L(D)}}$ above.
\end{enumerate}
...
\item To show this is well defined, first note that it is defined everywhere because $A$ is connected, so every state $s\in S_A$ has at least one string reaching it. It is not multiply-defined: suppose $x$ and $y$ both satisfy
...
\item First, take the candidate string for the start state to be
$x_{s_{0A}}=\lambda$. Then
\[
\psi(s_{0A})=\DBAR_B(s_{0B},x_{s_{0A}})
=\DBAR_B(s_{0B},\lambda)=s_{0B}.
\]
...
\mu([x]_R)=[x]_{R_L}.
\]
This is well defined: every $R$-class has at least one representative $x\in\Sigma^*$, so the definition applies everywhere on $S_{A_R}$. If $[x]_R=[y]_R$, then $xRy$, and since $R$ refines $R_L$, also $xR_Ly$, so $[x]_{R_L}=[y]_{R_L}$.

Now
\[
\mu(s_{0_{A_R}})=\mu([\lambda]_R)=[\lambda]_{R_L}=s_{0_{R_L}},
\]
so $\mu$ sends the start state of $A_R$ to the start state of $A_{R_L}$.
...
Let $[x]_Q$ be any state of $A_Q$; such an $x$ exists because equivalence classes are nonempty. Then $\DBAR_Q([\lambda]_Q,x)=[x]_Q$, so every state of $A_Q$ is reachable. $\square$
...
\begin{enumerate}
\item For each $R^A$-class $[x]_{R^A}$, choose a representative $x$ from the class; this is possible because equivalence classes are nonempty. Define $\psi([x]_{R^A})=\DBAR(s_0,x)$, which sends each class to the state it reaches.

\item Well-defined: $[x]_{R^A}=[y]_{R^A}\Rightarrow\DBAR(s_0,x)=\DBAR(s_0,y)$. \checkmark

\item Homomorphism: $\psi([\lambda]_{R^A})=s_0$. Also
\[
\psi([xa]_{R^A})=\DBAR(s_0,xa)=\delta(\DBAR(s_0,x),a)
=\delta(\psi([x]_{R^A}),a).
\]
Finally, since $F^c=F\cap S^c$ for the connected part of $A$,
\[
[x]\in F_{R^A}\Leftrightarrow x\in L(A)
\Leftrightarrow\DBAR(s_0,x)\in F
\Leftrightarrow\psi([x])\in F^c.
\]
\checkmark

\item Isomorphism: Injective: $\psi([x])=\psi([y])\Rightarrow\DBAR(s_0,x)=\DBAR(s_0,y)\Rightarrow[x]=[y]$. Surjective (onto $S^c$): if $s\in S^c$, then connectedness gives a string $x_s$ with $s=\DBAR(s_0,x_s)$, and therefore $s=\psi([x_s])$.
\end{enumerate}
...
$x_u\in\Sigma^*$ such that $\DBAR_B(s_{0B},x_u)=u$. Let
$s=\DBAR_A(s_{0A},x_u)\in S_A$. By Exercise 3.10,
\[
\psi(s)=\psi(\DBAR_A(s_{0A},x_u))
=\DBAR_B(\psi(s_{0A}),x_u)
=\DBAR_B(s_{0B},x_u)
=u.
\]
