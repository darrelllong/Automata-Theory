...
where
\[
\delta(q_0,a)=q_1,\qquad \delta(q_1,a)=q_0,\qquad \delta(q_2,a)=q_2.
\]
The state $q_2$ is unreachable, so $A$ is not minimal. From the initial state,
the machine accepts exactly the even-length strings, so
\[
L(A)=\{a^{2k}\mid k\geq 0\}.
\]
Therefore $R_{L(A)}$ has exactly two classes, represented by $\lambda$ and $a$.
But $R^A$ is the state congruence of the whole machine, so it has three classes,
represented by the three states $q_0,q_1,q_2$. Hence
\[
R^A\neq R_{L(A)}.
\]
