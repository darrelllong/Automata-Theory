...
\item $\{\lambda\}$ (the set containing only the empty word) is a regular set.
...
Lemma \ref{lmm-6.1}). Indeed, $\emptyset$ is the \emph{identity} for union and $\epsilon$
...
$\pmb{4}$ equals the string of symbols (that is, the word) $20 \div 5$; we
...
building blocks'' are recognizable. Figure \ref{fig-6.1} shows three NDFAs that accept
...
% Figure 6.1
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig6-1.eps,width=4in}}
\caption{NDFAs which recognize regular expressions with zero operators}\label{fig-6.1}
\end{figure}

% Figure 6.2
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig6-2.eps,width=2.5in}}
...
% Figure 6.3
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig6-3.eps,width=2.5in}}
...
% Figure 6.4
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig6-4.eps,width=3.5in}}
...
resulting machines are not advertised to be efficient; the main point is that
...
that a solution to this equation is a numerical value for $x$ that will make the
equation true, that is, make both sides equal. In the above example, there is
...
Here $X$ represents a set, and we are again looking for a value for $X$ that
...
know that the only solution is exactly $A^*E$.
\end{thm}
...
is not a solution). The equation $Z=\{\pmb{b},\pmb{c}\}\cup\{\pmb{a},\lambda\}
...
$X_2$ & $=$ & $E_2 \cup A_{21}X_1 \cup A_{22}X_2 \cup \cdots \cup A_{2(n-2)}
...
&& $\cup (A_{kn} \cdot (A^*_{nn} \cdot E_n \cup A^*_{nn} \cdot A_{n1}X_1 \cup
	\cdot A^*_{nn} \cdot A_{n2}X_2 \cup \cdots \cup \cdot A^*_{nn} \cdot
	A_{n(n-1)}X_{n-1}))$
\end{tabular} \\
...
or
\begin{center}\begin{tabular}{l}
$X_k = (E_k \cup A_{kn} \cdot A^*_{nn} \cdot E_n) \cup (A_{k1}X_1 \cup A_{kn}
...
where
\[ \hat{E}_1 = E_1 \cup (A_{12}\cdot A^*_{22}\cdot E_2) = E \cup (\pmb{b} \cdot \epsilon
...
Thus we have $X_1 = \epsilon \cup (\pmb{a} \cup \pmb{bb})$. $X_1$, which by
...
% Figure 6.5
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig6-5.eps,width=2.5in}}
...
\[ A_{ij} = \bigcup\limits_{\pmb{a}\,\in\,\Sigma\,\wedge\,s_j\,\in\,\delta(s_j,
	\pmb{a})}\pmb{a}\qquad\text{for }i,j=1,2, \ldots ,n \]

An $n$-state NDFA therefore gives rise to $n$ equation in $n$ unknowns, which
...
% Figure 6.6
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig6-6.eps,width=2.5in}}
...
% Figure 6.7
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig6-7.eps,width=2.5in}}
...
& X_3 = \pmb{a}X_1 \cup \pmb{c}X_1 \cup \pmb{c}X_2
\end{align*}
...
\begin{center}\begin{tabular}{r@{ = }l}
$X_1$ & $\epsilon\cup(\pmb{b}\cup\pmb{c}(\pmb{a}\cup\pmb{b}))X_2\cup(\pmb{a}\cup
	\pmb{cc})X_2$ \\
...
$\hat{\hat{\hat{A}}}_{11}$ in Example 6.11. The final constant term associated
...
% Figure 6.8
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig6-8.eps,width=2.5in}}
\caption{The NDFA discussed in Example 6.12}\label{fig-6.8}
\end{figure}
...
% Figure 6.9
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig6-9.eps,width=2.5in}}
...
in which $(\forall i,j)(\forall\lambda \notin A_{ij})$
...
% Figure 6.10
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig6-10.eps,width=2.5in}}
...
$X_1$ & $\emptyset\cup\emptyset X_1 \cup (\pmb{0}\cup\pmb{1})X_2\cup\emptyset
	X_3$ \\
$X_2$ & $\epsilon \cup \emptyset X_1 \cup \pmb{1}X_2 \cup \emptyset X_3$ \\
...
\[ Y_k = \hat{I}_k\cup Y_1B_{k1} \cup Y_2B_{k2} \cup\cdots\cup Y_nB_{kN},\qquad
	\text{for }k = 1,2,\ldots,n \]
in which $(\forall i,j)(\lambda \notin B_{ij})$.
\begin{enumerate}[a.]
\item The unknown $Y_m$ can be eliminated from this system to form the following
$n-1$ equations in the unknowns $Y_1,Y_2, \ldots , Y_{m-l},Y_{m+l},\ldots,Y_n$.
...
$\Gamma$ be a second alphabet. Given regular expressions $R_1,R_2,\ldots,R_n$
...
\Sigma^*$ by defining
\[ \overline{s}(L) = \bigcup\limits_{z\,\in\, L} s(z) \]
...
concatenation of the two corresponding substitutions. That is, $\overline{s}(R
...
language homomorphism. Then \RSIG\ closed under \PBAR.
...
\item Let $\Sigma=\{\pmb{a},\pmb{b},\pmb{c}\}$. Give a regular expression that
describes the set of all words in $\Sigma^*$ that do not contain two consecutive
$\pmb{c}$s. 

\item Let $\Sigma=\{\pmb{a},\pmb{b},\pmb{c}\}$. Give a regular expression that
describes the set of all words in $\Sigma^*$ that do contain two consecutive
$\pmb{c}$s.

\item Let $\Sigma=\{\pmb{a},\pmb{b},\pmb{c}\}$. Give a regular expression that
describes the set of all words in $\Sigma^*$ that do not contain any $\pmb{c}$s.
...
\begin{center}\begin{tabular}{r@{ = }l}
$r$ & $\{x \in \Sigma^*|\ $the number represented by $x$ is evenly
	divisible by $3\}$ \\
...
\item Use the exact constructs given in the theorems of Chapter \ref{ch-5} to build a
...
\item $R_1 \cdot R_2 \not= R_2 \cdot R_1$
\item $R_1 \cdot R_1 \not= R_1$
\item $R_1 \cup (R_2 \cdot R_3) \not= (R_1 \cup R_2)\cdot(R_l \cup R_3)$
\item $(R_1 \cdot R_2)^* \not= (R^*_1 \cdot R^*_1)^*$
\item $(R_1 \cdot R_2)^* \not= (R^*_1 \cup R^*_2)^*$
\end{enumerate}
...
\setcounter{enumii}{6}
\item $R_1 \cup \epsilon \notin R_1$
\item $R_1 \cdot R_2 \not= R_2 \cdot R_1\qquad$ (even if $R_1 \not= R_2$)
\item $R_1 \cdot R_1 \not= R_1$
\item $R_1 \cup (R_2 \cdot R_3) \not= (R_1 \cup R_2)\cdot(R_l \cup R_3)\qquad$
	(even if $R_1 \not= R_2 \not= R_3 \not= R_1$)
\item $(R_1 \cdot R_2)^* \not= (R^*_1 \cdot R^*_1)^*\qquad$ (even if $R_1 \not=
	R_2$)
\item $(R_1 \cdot R_2)^* \not= (R^*_1 \cup R^*_2)^*\qquad$ (even if $R_1 \not=
	R_2$)
\end{enumerate}
...
over $\{\pmb{0},\pmb{1}\}^*$:
\begin{center}\begin{tabular}{r@{ = }l}
$X_1$ & $(\pmb{0}\cup\pmb{1})X_1$ \\
$X_2$ & $\epsilon\cup\pmb{1}X_0\cup\pmb{0}X_1$
\end{tabular}\end{center}
Do you see any relation between these equations and the DFA $A$ in Example 3.4?

\item \begin{enumerate}
\item Solve the following set of language equations for $X_1$, $X_2$, and $X_3$
...
\item Give a regular expression that will describe the \emph{intersection} of
...
\begin{center}\begin{tabular}{r@{ = }l}
$X_1$ & $\epsilon\cup\pmb{a}\cdot X_1\cup\pmb{b}\cdot X_2$ \\
$X_2$ & $\emptyset\cup\pmb{b}\cdot X_2 \cup\emptyset\cdot X_2$
\end{tabular}\end{center}

\item \begin{enumerate}
\item Find $L(D)$ for the machine $D$ described in Example 6.10.
\item Generalize your technique: For a machine $A$ with start states $s_1,\ s_2,
...
$X_2$ & $\pmb{c}\cup\epsilon\cdot X_1 \cup\epsilon\cdot X_2$
\end{tabular}\end{center}
\item Does this contradict Theorem \ref{thm-6.2}? Explain.
\end{enumerate}

\item Solve the following set of language equations for $X_0$ and $X_1$ over
$\{\pmb{0},\pmb{1}\}^*$:
\begin{center}\begin{tabular}{r@{ = }l@{ $\cup$ }l@{ $\cup$ }l}
$X_0$ & $\pmb{0}^*\pmb{1}$ & $(\pmb{10})^*X_0$ & $\pmb{0}(\pmb{0}\cup\pmb{1})X_2$ \\
...
\item Recall that the reverse of a word $x$, written $x'$, is the word written
backward. The reverse of a language is likewise given by $L'=\{x'\ |\ x \in L\}$.
Let $\Sigma=\{\pmb{a},\pmb{b},\pmb{c}\}$.
\begin{enumerate}
\item Note that $(R_1 \cup R_2)' = (R'_1 \cup R'_2)$ for any regular sets $R_1$
...
regular expression representing $L'$ (compare with the technique used in the
...
% Figure 6.11
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig6-11.eps,width=2.5in}}
...
% Figure 6.12
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig6-12.eps,width=4.5in}}
...
% Figure 6.13
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig6-13.eps,width=6.5in}}
...
is a language over the alphabet $\{\pmb{a},\pmb{b},(,),\cup, ^*,\emptyset,
...
% Figure 6.14
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig6-14.eps,width=6.2in}}
\caption{The automata for Exercise 6.50}\label{fig-6.14}
\end{figure}

\item Let $F$ be the collection of languages that can be formed by repeated
application of the following five rules:
\begin{enumerate}[i.]
\item $\{\pmb{a}\}\in F$ and $\{\pmb{b}\}\in F$
\item $\{\ \}\in F$
\item $\{\lambda\}\in F$
\item If $F_1\in F_2$ and $F_2 \in F$, then $F_1 \cdot F_2 \in F$
\item If $F_1 \in F$ and $F_2 \in F$, then $F_1 \cup F_2 \in F$
\end{enumerate}
