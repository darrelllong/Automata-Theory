...
% Figure 11.1
\begin{figure}[tbh]
\centerline{\epsfig{figure=ToFAfigures/fig11-1.eps,width=3.4in}}
...
$\Gamma$ is the \emph{auxiliary alphabet}, and $\Sigma$, $\Gamma$, and $\{L,R\}$
...
state $h$ is a special \Index{halt state}\index{Turing machine!halt state}, from which no further transitions are
possible; $h\notin S$.
\end{dfn}
...
% Figure 11.2
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig11-2.eps,width=2.4in}}
...
they function much like a return statement in many programming languages.
Example 11.4 illustrates a Turing machine that employs submachines.

% Figure 11.3
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig11-3.eps,width=2.4in}}
...
States $s_0$ and $s_1$ in Figure \ref{fig-11.4} check the while condition, and states $s_2$
...
% Figure 11.4
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig11-4.eps,height=3.4in}}
...
operating on a tape containing\linebreak$\ldots\#\#\#\alpha\pmb{b}\beta\#\#\#\ldots$,
...
\#\#t\#}$ indicates that the tape head has moved past the word $\pmb{ac}$ and is
...
\pmb{\#},\beta=\pmb{\#ac}$). A totally blank tape\index{tape!blank} is represented by $t\#$.
...
It should be clear that a Turing machine $A$, accepting $L=L_1(A_1)$ has an
...
% Figure 11.5a
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig11-5a.eps,height=4.0in}}
\caption{The Turing machine discussed in Example 11.4}\label{fig-11.5a}
\end{figure}

% Figure 11.5b
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig11-5b.eps,height=1.2in}}
\caption{Submachine $S_1$ in Example 11.4}\label{fig-11.5b}
\end{figure}

% Figure 11.5c
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig11-5c.eps,height=1.2in}}
...
\[ (\forall x\in\Sigma^*)(s_0x\Dvdash xq\#\text{ iff }\DBAR_A(s_0,x)=q) \]
Therefore,
...
% Figure 11.6
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig11-6.eps,width=2.4in}}
...
\centerline{\epsfig{figure=ToFAfigures/fig11-7.eps,width=3.4in}}
...
% Figure 11.8
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig11-8.eps,width=3.4in}}
...
\centerline{\epsfig{figure=ToFAfigures/fig11-9.eps,width=3.4in}}
\caption{A three-tape Turing machine}\label{fig-11.9}
\end{figure}

% Figure 11.10
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig11-10.eps,width=3.4in}}
...
Let $\Sigma$ be an alphabet. Then $\PSIG\subseteq\TSIG$. That is, every
...
% Figure 11.11
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig11-11.eps,width=3.4in}}
...
% Figure 11.12
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig11-12.eps,width=3.4in}}
...
% Figure 11.13
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig11-13.eps,height=3.8in}}
...
&&
	$\vdash$ \\
$\pmb{babccahY}$ \end{tabular}\end{center}
...
respectively. Attempting to move left past the conceptual position of \pmb{[} (or
right past the position of \pmb{]}) will result in the addition of a blank symbol to
the sentential form.
...
$(\forall a\in\Sigma)([W\rightarrow[W\pmb{a}\in P_M)$ \\
...
of the class 2 productions can then be used. Only if $Y$ was written to tape as
...
\begin{center}\begin{tabular}{l}
$[\pmb{\#babccahY}]\Rightarrow[\pmb{\#babcca}U]\Rightarrow[\pmb{\#babcca}V
...
\[ (\forall\alpha,\beta,\gamma,\omega\in(\Sigma\cup\Gamma)^*)(s_0x\Dvdash x
	\pmb{hY}\text{ iff }(\exists m,n\in\NN)([s_0x\pmb{\#}]\ \DRightarrow
	[\pmb{\#}^mx\pmb{hY\#}^n])) \]
...
% Figure 11.14a
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig11-14a.eps,width=3.4in}}
\caption{A three-track Turing machine employing delimiters}\label{fig-11.14a}
\end{figure}

% Figure 11.14b
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig11-14b.eps,width=3.4in}}
...
% Figure 11.15
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig11-15.eps,height=3.8in}}
...
$\overline{\pmb{B}}\pmb{A}\underline{\pmb{a}}$ is obtained. At this step, no production will
...
machine $\delta$ and words generated by the context-sensitive grammar $G_B$
