% Chapter, Section, Theorem, Corollary, Lemma, Definition, Table, Figure
...
Furthermore, if $(\alpha_1\Rightarrow\alpha_2)\wedge(\alpha_2\Rightarrow\alpha_3)
\wedge\cdots\wedge(\alpha_{n-1}\Rightarrow\alpha_n)$, then we will say that
$\alpha_1$ \emph{derives} $\alpha_n$ and write $\alpha_1 \DRightarrow\alpha_n$.
\end{dfn}
...
% Figure 9.1
\begin{figure}
\centerline{\includegraphics[width=5.5in]{ToFAfigures/fig9-1.eps}}
...
% Figure 9.2
\begin{figure}
\centerline{\includegraphics[width=4.5in]{ToFAfigures/fig9-2.eps}}
...
constructed. In the case of a string like $\pmb{((a\cup\cdot b)}$, no such parse
...
$\pmb{((}\pmb{a}{\pmb{\cup}}\pmb{b})\pmb{^*\cdot}\pmb{c})$ in Figure \ref{fig-9.3} clearly shows both the
order in which the operators $\pmb{\cup}$, $\pmb{\cdot}$, and $\pmb{^*}$ should be applied and the
expressions to which they should be applied.

Given a particular complete parse tree for a string $x$, there may be some
freedom in the \emph{order} in which the associated productions are applied.

% Figure 9.3
\begin{figure}
\centerline{\includegraphics[width=6.0in]{ToFAfigures/fig9-3.eps}}
...
%	R)|R^*\} \]
each of the following \textcolor{red}{are} valid derivations of the string $x=\pmb{((a\cup b)^*  \cdot c)}$.

\noindent\emph{Derivation 1:}
\begin{center}\begin{tabular}{r@{ $\Rightarrow$ }l}
$R$ & $\pmb{(}R\pmb{\cdot} R\pmb)$ \\
& $\pmb{(}R\pmb{^*\cdot} R\pmb{)}$ \\
& $\pmb{((}R\pmb{\cup} R\pmb{)^*}\pmb{\cdot} R\pmb{)}$ \\
& $\pmb{((}\pmb{a\cup} R\pmb{)^*\cdot} R\pmb{)}$ \\
& $\pmb{((}\pmb{a\cup b}\pmb{)^*\cdot} R\pmb{)}$ \\
& $\pmb{((a}\pmb{\cup}\pmb{b)}\pmb{^*\cdot}\pmb{c)}$
\end{tabular}\end{center}
\emph{Derivation 2:}
\begin{center}\begin{tabular}{r@{ $\Rightarrow$ }l}
$R$ & $\pmb{(}R\pmb{\cdot} R\pmb{)}$ \\
& $\pmb{(}R\pmb{^*\cdot} R\pmb{)}$ \\
& $\pmb{((}R{\pmb{\cup}} R\pmb{)^*\cdot} R\pmb{)}$ \\
& $\pmb{((}R{\pmb{\cup}} R\pmb{)^*\cdot c)}$ \\
& $\pmb{((}R{\pmb{\cup}}\pmb{b)^*\cdot c)}$ \\
& $\pmb{((}\pmb{a}{\pmb{\cup}}\pmb{b})\pmb{^*\cdot}\pmb{c})$
\end{tabular}\end{center}
\emph{Derivation 3:}
\begin{center}\begin{tabular}{r@{ $\Rightarrow$ }l}
$R$ & $\pmb{(}R\pmb{\cdot} R\pmb{)}$ \\
...
& $\pmb{(}R\pmb{^*\cdot c)}$ \\
& $\pmb{((}R\pmb{\cup} R\pmb{)^*\cdot c)}$ \\
& $\pmb{((}\pmb{a}\pmb{\cup} R\pmb{)^*\cdot c)}$ \\
& $\pmb{((a}\pmb{\cup}\pmb{b)}\pmb{^*\cdot}\pmb{c)}$
\end{tabular}\end{center}
\emph{Derivation 4:}
\begin{center}\begin{tabular}{r@{ $\Rightarrow$ }l}
...
the same sequence, since there is never a choice of nonterminals to expand.
...
the nodes in the parse tree for $\pmb{((a\cup b)^*  \cdot c)}$ are
...
% Figure 9.4
\begin{figure}
\centerline{\includegraphics[width=6.0in]{ToFAfigures/fig9-4.eps}}
...
% Figure 9.5
\begin{figure}
\centerline{\includegraphics[height=5.5in]{ToFAfigures/fig9-5.eps}}
...
\<expression\> & \<identifier\> \pmb{--} \<expression\> \\
\<expression\> & \<expression\> \pmb{--} \<identifier\> \\
\<identifier\> & $\pmb{a}$ \\
\<identifier\> & $\pmb{b}$ \\
\<identifier\> & $\pmb{c}$ \\
\<identifier\> & $\pmb{d}$
...
\begin{center}\begin{tabular}{r@{ $\Rightarrow$ }l}
\<expression\> & \<expression\> \pmb{--} \<identifier\> \\
& \<identifier\> \pmb{--} \<expression\> \pmb{--} \<identifier\> \\
& $\pmb{a-}$ \<expression\> \pmb{--} \<identifier\> \\
& $\pmb{a-}$ \<identifier\> \pmb{--} \<identifier\> \\
& $\pmb{a-b-}$ \<identifier\> \\
& $\pmb{a-b-d}$
\end{tabular}\end{center}
...
\<expression\> & \<identifier\> \pmb{--} \<expression\> \\
& $\pmb{a-}$ \<expression\> \\
& $\pmb{a-}$ \<identifier\> \pmb{--} \<expression\> \\
& $\pmb{a-b-}$ \<expression\> \\
& $\pmb{a-b-}$ \<identifier\> \\
& $\pmb{a-b-d}$
\end{tabular}\end{center}
...
\[ \text{\<expression\>}\rightarrow\text{\<identifier\>}\pmb{-}\text{\<expression\>}\]
and
\[ \text{\<expression\>}\rightarrow\text{\<expression\>}\pmb{-}\text{\<identifier\>}\]
...
% Figure 9.6
\begin{figure}
\centerline{\includegraphics[height=6.5in]{ToFAfigures/fig9-6.eps}}
...
languages, the expression $\pmb{a-b-c}$ is allowed and has a well-defined
meaning. Most languages decree that such expressions be evaluated from left to
right, and hence $\pmb{a-b-c}$ would be interpreted as ($\pmb{a-b}$)$\pmb{-c}$.
...
T\rightarrow\pmb{a}|\pmb{b}|\pmb{c}|\pmb{d}|T \pmb{-} T\}\rangle$. However, since
...
reflect the meaning of expressions like $\pmb{d-(b-c)}$ or even $\pmb{(a)-((c-(d)))}$ are
sought.
...
where $P''$ consists of the productions
\begin{center}\begin{tabular}{r@{ $\rightarrow$ }l}
\<expression\> & \pmb{(}\<expression\>\pmb{)} \\
\<expression\> & \<expression\> \pmb{--} \pmb{(}\<expression\>\pmb{)} \\
\<expression\> & \<identifier\> \\
\<expression\> & \<expression\> \pmb{--} \<identifier\> \\
\<identifier\> & $\pmb{a}$ \\
\<identifier\> & $\pmb{b}$ \\
...
the unique parse tree for the expression $\pmb{(a)-((c-(d)))}$.

$G_P$ is a context-free language that is too complex to be regular; the pumping
lemma for regular sets (Theorem \ref{thm-2.3}) can be used to show \textcolor{red}{that} is impossible for
...
% Figure 9.7
\begin{figure}
\centerline{\includegraphics[height=3.5in]{ToFAfigures/fig9-7.eps}}
...
% Figure 9.8
\begin{figure}
\centerline{\includegraphics[width=5.0in]{ToFAfigures/fig9-8.eps}}
...
% Figure 9.9
\begin{figure}
\centerline{\includegraphics[height=7.5in]{ToFAfigures/fig9-9.eps}}
...
the corresponding parse tree. The partial parse tree for a string such as \textbf{abb} would be as pictured in Figure \ref{fig-9.10}a. This would clearly be an invalid
...
string causes a check for whether the current nonterminal can produce the empty
...
% Figure 9.10
\begin{figure}
\centerline{\includegraphics[height=3.7in]{ToFAfigures/fig9-10.eps}}
...
simple enough to be defined by a grammar that \emph{is} LL0, since every regular
...
should cause the variable $\mathtt{DO77I}$ to be assigned the value
$\mathtt{1.5}$. On the other hand, the statement
\[ \mathtt{DO\ 77\ I=\ 1,5} \]
...
\rightarrow\pmb{oo}C$, and $C\rightarrow\pmb{gi}$.
...
\centerline{\includegraphics[width=4.7in]{ToFAfigures/fig9-11.eps}}
...
will follow immediately from Definition \ref{dfn-8.6}. If \textcolor{red}{L} is a pure context-free
...
terminal, then the rule must be modified by replacing any terminal symbol {\pmb
...
term. Since the set of all $B$-rules \textcolor{red}{were} $B\rightarrow\beta_1,B\rightarrow
...
and the set of all \textcolor{red}{non[left]recursive X-rules} by $X^n=\{X\rightarrow\gamma_1,X
\rightarrow\gamma_2,\ldots,X\rightarrow\gamma_n\}$. Choose a new nonterminal $Y
\not in\Omega$ and let $G'' = \langle\Omega\cup\{Y\},\Sigma,S,P''\rangle$, where
...
$\{X\rightarrow \gamma_1Y,X\rightarrow\gamma_2Y,\ldots,X\rightarrow\gamma_nY\}
	\cup\{Y\rightarrow\alpha_1,Y\rightarrow\alpha_2,\ldots,Y\rightarrow
	\alpha_m\}\cup$ \\
...
\rightarrow S_j\alpha$ is found where $i<j$\textcolor{red}{. eliminate} it as specified in Lemma
...
\section{Pumping Theorem}\label{sec-9.4}
...
% Figure 9.12
\begin{figure}
\centerline{\includegraphics[height=3.7in]{ToFAfigures/fig9-12.eps}}
\caption{The parse tree discussed in the proof of Theorem \ref{thm-9.7}}\label{fig-9.12}
\end{figure}

The two occurrences of \textcolor{red}{R} were in distinct places in the parse tree, and hence at
...
$|v|+|x|>0$. Furthermore, the subtree rooted at the higher occurrence of \textcolor{red}{R} was
...
\textbf{if} $(\forall n\in\NN)(\exists z\in L \ni |z| \geq n)(\forall u,v,w,x,y \in
\Sigma^* \ni z = uvwxy, |vwx| \leq n, |vx|\geq 1)(\exists i\in\NN \ni uv^iwx^iy
\not in L)$ \\
\textbf{then} $L$ is \textbf{not} context free.
...
The language $L=\{\pmb{a}^k\pmb{b}^k\pmb{c}^k|\ k\in\NN\}$ is not a context-free language. Let $n$
be given, and choose $z=\pmb{a}^n\pmb{b}^n\pmb{c}^n$. Then $z\in L$ and $|z|= 3n \geq n$. If $L$
...
If the strings $v$ and $x$ contain only one type of letter (for example, ${\pmb
c}$), then $uv^2wx^2y$ will contain more $\pmb{c}$s than $\pmb{a}$s or $\pmb{b}$s, and
thus $uv^2wx^2y \not in L$. If $v$ were, say, all $\pmb{b}$s and $x$ were all ${\pmb
...
$uv^2wx^2y \not in L$. All other cases are similar to these, and they
...
since \emph{all} combinations of $u,\ v,\ w,\ x$, and $y$ had to be shown to lead to
...
\section{Closure Properties}\label{sec-9.5}
...
definition of \emph{substitution} for context-free languages.
...
by Definition \ref{dfn-6.5}. For context-free languages, \emph{substitution} denotes the
consistent replacement of the individual letters within each word of a
context-free language with \emph{sets} of words. Each such set of words must also be a
...
\Sigma,S,P\rangle$ that generates $L$. If $s$: $\Sigma\rightarrow\Sigma^*$ is
a substitution satisfying Definition \ref{dfn-9.11}, then for each letter $\pmb{a}_k\in
\Sigma$ there is a corresponding grammar $G_k=\langle\Omega_k,\Sigma,S_k,P_k
...
The grammar $\pmb{\cup} = \langle\{S\},\{\pmb{a},\pmb{b}\},S,
\{S  \rightarrow  \pmb{a},S  \rightarrow  \pmb{b}\}\rangle$
clearly generates the context-free language
...
\not in\Omega_1$ and $S'\not in\Omega_1$, and define a new grammar
...
Chapter \ref{ch-5} for regular languages carry over to the class of context-free
languages. Closure under intersection does \emph{not} extend, as the next result shows.
...
the time $m$ reaches $n^2$\textcolor{red}{ .} The words in $L$ of length less than $n$ constitute
a finite set $F$, which is regular. Each $L_k$ is represented by the regular
expression indicated by $(\pmb{a}^{i_k})^*\cdot\pmb{a}^{j_k}$\textcolor{red}{ ,} and there are \textcolor{red}{less}
...
\emptyset$. Choose a new nonterminal $Z$ such that $Z\not in\Omega_1\cup\Omega_2$,
and define a new grammar $G^{\cdot} = \langle\Omega_1\cup\Omega_2\cup\{Z\},\Sigma,Z,
P_1\cup P_2\cup\{Z\rightarrow S_1\cdot S_2\}\rangle$. \emph{Note:} It \emph{is}
straightforward to show that $L(G^{\cdot}) = L(G_1)\cdot L(G_2)$.
\item Modify $G^{\cdot}$ so that it reflects an appropriate valid context-free
...
\[ P' = P\cup\{X\rightarrow\gamma\beta_1\alpha,X\rightarrow\gamma\beta_2
	\alpha,\ldots,X\rightarrow\gamma\beta_m\alpha\}-\{X\rightarrow\gamma
	B\alpha\}. \]
Then $L(G) = L(G')$.
...
\item Determine if \XSIG, the set of all languages that are \emph{not} context
free,\index{X@\XSIG \ (X-sigma )} is closed under:
\begin{enumerate}
...
\item Characterize the languages generated by grammars that \emph{can} be represented
by this restrictive form.
\end{enumerate}

\item Let $L$ be \emph{any} collection of words over an alphabet $\Sigma$. Prove
or disprove that $L^*$ must be regular.
