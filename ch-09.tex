...
%	R)|R^*\} \]
the following are each valid derivations of the string $x=\pmb{((a\cup b)^*  \cdot c)}$.
...
lemma for regular sets (Theorem \ref{thm-2.3}) can be used to show that it is impossible for
...
term. Since the set of all $B$-rules are $B\rightarrow\beta_1,B\rightarrow
\beta_2,\ldots,B\rightarrow\beta_m$, then $X_B$ satisfies the equation
...
\end{tabular}\end{center}
together with the original non-left-recursive $X$-rules, can replace the
left-recursive $X$-rules $X\rightarrow X\alpha_1,\ X\rightarrow X\alpha_2,
\ldots,X\rightarrow X\alpha_m$.
\end{lmm}
