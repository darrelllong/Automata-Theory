...
By the pumping lemma: $s = uvwxy$, $|vwx| \leq n$, $|vx| = m \geq 1$. Then pumped strings have length $j^2 + (i-1)m$. For $i=2$: length $j^2 + m$. We need $j^2 + m$ to be a perfect square. But $j^2 < j^2 + m \leq j^2 + n < j^2 + 2j + 1 = (j+1)^2$ (since $n < j^2 \leq j^2$ and we can choose $j$ large enough that $n < 2j$). So $j^2 + m$ is strictly between two consecutive perfect squares, hence not a perfect square. Contradiction. \qed
...
\begin{enumerate}
\item \textbf{Union}: Not closed. $L_1 = \{\pmb{a}^n\pmb{b}^n\pmb{c}^m \mid n\neq m\}$ and $L_2 = \{\pmb{a}^n\pmb{b}^m\pmb{c}^m \mid n\neq m\}$ are in \XSIG\ (not context free), but $L_1 \cup L_2 \supseteq \{\pmb{a}^n\pmb{b}^n\pmb{c}^n\}$ and $L_1\cup L_2 = \{a^ib^jc^k \mid i\neq k \text{ or } j\neq k\}$ which is not context free. But also $\Sigma^*$ (CFL) is the complement of $\emptyset$ (CFL)... a simpler counterexample: $\overline{L} \cup L = \Sigma^*$ is a CFL, so if both $L$ and $\overline{L}$ are in \XSIG, their union is a CFL $\notin$ \XSIG. So \XSIG\ is \emph{not} closed under union.
...
Since marked positions are all in the $\pmb{b}$-block, $vwx$ lies within the $\pmb{b}$-block (at most). Then $v$ and $x$ consist only of $\pmb{b}$s (or $\pmb{a}$s on the boundary, or $\pmb{c}$s on the boundary). If $vx$ contains only $\pmb{b}$s, pumping changes $|w|_{\pmb{b}}$ while $|w|_{\pmb{a}} = p!+p$ and $|w|_{\pmb{c}} = p!+p$ remain fixed. At some $i$, $|w|_{\pmb{b}} = p!+p$ (i.e., $p + (i-1)|vx| = p!+p$, so $(i-1)|vx| = p!$, giving $i-1 = p!/|vx|$ which is an integer since $|vx| \leq p$ divides $p!$). But then $k = n = p!+p$ and $n = m = p!+p$, so the pumped string is not in $L$ (requires $k\neq n$ and $n\neq m$). Contradiction. \qed
