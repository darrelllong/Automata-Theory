...
\begin{enumerate}[(a)]
  ...
  \item \textbf{Union: No.}  $L_1=\{\pmb{a}^{2n}\mid n\ge 0\}$ (infinite but not cofinite) is not in $B_\Sigma$.  But for $L_1,L_2\in B_\Sigma$: if both finite, union is finite.  If one is cofinite, union is cofinite.  If both cofinite, union is cofinite.  \textbf{Yes, closed.}
...
\begin{enumerate}[(a)]
  ...
  \item \textbf{Concatenation: No.}  Counterexample needed; this is complex. $\emptyset\in J_\Sigma$ but $\emptyset\cdot L=\emptyset$; $\sim\emptyset=\Sigma^*$ is infinite so $\emptyset\in J_\Sigma$ and $\emptyset\cdot L=\emptyset\in J_\Sigma$.  Let $L_1=\{\pmb{a}\}^*$ (complement contains all words with a $\pmb{b}$, infinite if $|\Sigma|\ge 2$): $L_1\in J_\Sigma$.  $L_1\cdot L_1=L_1$.  This doesn't give a counterexample easily.  \textbf{Not determined here; left as investigation.}
  \item \textbf{Kleene closure: No.}  $\emptyset^*=\{\lambda\}$.  $\sim\{\lambda\}=\Sigma^*\setminus\{\lambda\}$ is infinite, so $\{\lambda\}\in J_\Sigma$.  But $\emptyset\in J_\Sigma$ and $\emptyset^*=\{\lambda\}\in J_\Sigma$---this is closed.  Let $L=\{\lambda\}$: $L^*=\{\lambda\}$, still in $J_\Sigma$.  Counterexample: hard to find; likely \textbf{closed}.
  \item \textbf{Relative complement: No.}  $L_1\setminus L_2=L_1\cap\sim L_2$.  $\sim L_2$ could be very large; hard to get control.
\end{enumerate}
...
Let $\mathcal{C}$ be the class of FAD (regular) languages.  This is closed under union, concatenation, and Kleene closure.  But... it is also closed under intersection.  We need a different example.

Let $\mathcal{C}$ consist of all languages of the form $R^*$ for $R$ a regular expression (i.e., Kleene closures of FAD languages).  Every such language is FAD; closed under Kleene closure ($({R^*})^*=R^*$); closed under concatenation ($R_1^*\cdot R_2^*\subseteq(R_1|R_2)^*$ and can be shown FAD).
Note: the identity $R_1^*\cup R_2^*=(R_1|R_2)^*$ is \emph{false} in general (e.g., $R_1=\mathbf{a}$, $R_2=\mathbf{b}$: the left side is $\mathbf{a}^*\cup\mathbf{b}^*$ while the right is $(\mathbf{a}|\mathbf{b})^*$, and $\mathbf{ab}$ is in the right but not the left).
But intersection: $\{a\}^*\cap\{b\}^*=\{\lambda\}=(\lambda)^*$, which is in $\mathcal{C}$; this particular case closes. The collection $\mathcal{C}$ is not a clean counterexample here.

\textbf{A non-trivial example:} the collection of all context-free languages is closed under union and concatenation but not under intersection.
