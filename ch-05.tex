...
Let $\rho =\{X$ $|$ X is a finite subset of $\NN\}$; then $\rho$ is closed under
...
$\NN$ is not closed under $\sqrt{\ \ \ }$ (square root) since $7 \in \NN$ but
$\sqrt{7} \notin \NN$.
...
machines can easily be employed to construct a nondeterministic finite automaton
...
implies that the NDFA in Figure \ref{fig-5.2} will accept a string if any path leads to a
...
only strings that will be accepted. This trick is the basis of the following
...
% Figure 5.1
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-1.eps,width=3in}}
\caption{The two automata discussed in Example 5.6}\label{fig-5.1}
\end{figure}

% Figure 5.2
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-2.eps,width=3in}}
...
defined by
\[ \delta^\cup(s,a) = \left\{\begin{array}{lr}
				\delta_1(s,a) & \text{if }s \in S_1 \\
				& , \\
				\delta_2(s,a) & \text{if }s \in S_2
			     \end{array}\right.
\quad \forall s \in S_1 \cup S_2,\quad \forall a \in \Sigma \]
...
Given deterministic finite automata
$A_1 =\langle\Sigma,S_1,s_{0_1},\delta_1,F_1\rangle$ and
$A_2 =\langle\Sigma,S_2,s_{0_2},\delta_2,F_2\rangle$ such that $L(A_1)=L_1$, and
...
defined by
\[ \delta^\cap(\langle s,t\rangle,a) = \langle\delta_1(s,a),\delta_2(t,a)\rangle\quad
	\forall s \in S_1, \forall t \in S_2, \forall a \in \Sigma \]
...
% Figure 5.3
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-3.eps,height=3.6in}}
...
states of $A_1$ and $A_2$, and hence $s_0^\cap =\langle s_{0_1},s_{0_3}\rangle$.
...
% Figure 5.4
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-4.eps,height=3.6in}}
...
% Figure 5.5
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-5.eps,width=6in}}
\caption{Two candidates for concatenation}\label{fig-5.5}
\end{figure}

% Figure 5.6
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-6.eps,width=6in}}
...
% Figure 5.7
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-7.eps,width=6in}}
\caption{A pair of candidates for concatenation}\label{fig-5.7}
\end{figure}

% Figure 5.8
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-8.eps,width=6in}}
...
\begin{center}
\begin{tabular}{l}
$\delta^\bullet(s,a)=\left\{\begin{array}{ll}
		\{\delta_1(s,a)\} & \text{if }s \in S_1 - F_1 \\
		\{\delta_1(s,a),\delta_2(s_{0_2},a)\} & \text{if }s\in F_1 \\
		\{\delta_2(s,a)\} & \text{if }s \in S_2
		\end{array}\right.$
\end{tabular}
\end{center}
...
% Figure 5.9
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-9.eps,width=6in}}
...
% Figure 5.10
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-10.eps,width=6in}}
...
% Figure 5.11
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-11.eps,width=6in}}
...
% Figure 5.12
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-12.eps,width=6in}}
...
$L^+= \bigcup\limits^\infty_{i=1} L^i=L \cup L^2 \cup L^3 \cup \cdots =
...
$\{b,c,d\}$) for which each occurrence of $\pmb{d}$ is immediately followed by (at
least) one $\pmb{b}$.
...
\emph{Proof.} Let L belong to $\DSIG$. Then there is a nondeterministic
finite automaton $A =\langle\Sigma,S,s_0,\delta,F\rangle$ such that $L(A)=L$.
Define a nondeterministic machine
...
\[ \delta_{*}(s,a) = \left\{\begin{array}{ll}
		\delta(s,a) & \text{if }s \notin F \cup \{q_0\} \\
		\delta(s,a)\cup\left(\bigcup\limits_{t\in S_0}\delta(t,a)\right)
			& \text{if }s \in F \\
...
% Figure 5.13
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-13.eps,width=4in}}
\caption{The NDFA $B$ in Example 5.16.}\label{fig-5.13}
\end{figure}

% Figure 5.14
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-14.eps,width=4in}}
...
adhere to the nondeterministic definition and produce a set of states. As a
consequence, the definition of $\delta^\bullet$ involved expressions like
$\{\delta_1(s,a)\}$, which indicated that the single state given by
$\delta_1(s,a)$ should be viewed as a singleton set.
...
$\delta_*(s,a) = \delta(s,a)$. In this case, no set brackets $\{\ \}$
were necessary since $\delta(s,a)$ by assumption already represented a set
(rather than just a single element as in the deterministic case).

The definition of the new set of start states So is also affected by the type
...
accepts the language $\{a^nb^m\ |\ n \geq 1,m \geq 1\}.\ Z(L(C))$ would then be
\[ \{a^nb^m\ |\ n \geq 0, m \geq 0\} \]
...
% Figure 5.15
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-15.eps,width=5in}}
\caption{The automaton $C$ discussed in Example 5.17}\label{fig-5.15}
\end{figure}

% Figure 5.16
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-16.eps,width=5in}}
\caption{An automaton accepting $Z(C)$ in Example 5.17}\label{fig-5.16}
\end{figure}

% Figure 5.17
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-17.eps,width=4in}}
...
% Figure 5.18
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-18.eps,width=4in}}
...
\[ \NSIG =\{L \subseteq \Sigma^*| \text{ there does \emph{not} exist any finite
automaton } M \ni L(M) = L\} \]
\end{dfn}
...
$\mathfrak{N}_{\{a,b\}}$ is \emph{not} closed under $\cap$.

\emph{Proof.} Let $L_1=\{a^p\ |\ p$ is prime$\}$ and let $L_2=\{b^p\ |\ p$ is prime$\}$.
Then $L_1 \in \mathfrak{N}_{\{a,b\}}$, $L_2 \in \mathfrak{N}_{\{a,b\}}$ but
$L_1 \cap L_2 = \emptyset \notin \mathfrak{N}_{\{a,b\}}$ (why?).
\end{lmm}
...
$\psi(a_i) = w_i$ for each $i$, which can be extended to
$\PBAR$$: \Sigma^* \rightarrow \Gamma^*$ by:
\begin{center}
\begin{tabular}{rcl}
$\PBAR(\lambda)$ & $=$ & $\lambda$ \\
$(\forall a \in \Sigma)(\forall x \in \Sigma^*)(\PBAR(a\cdot x)$ & $=$ &
	$\psi(a) \cdot (\PBAR(x)))$
\end{tabular}
\end{center}
...
% Figure 5.19
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-19.eps,width=5.5in}}
...
% Figure 5.20
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-20.eps,width=5.5in}}
...
defined by $(\forall a \in \Sigma)(\mu(\pmb{a})=\pmb{a})$. Since
...
defined by $\xi(a)=\pmb{a}$ and $\xi(b)=\pmb{a}$. Then \NSIG\ is \emph{not}
...
\[ \delta^\psi(s,\pmb{a})=\DBAR\!(s,\psi(\pmb{a}))\ 
		\forall s \in S, \forall a \in \Sigma \]
...
% Figure 5.21
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig5-21.eps,width=6in}}
...
Let $\Sigma$ be an alphabet. Then \NSIG\ is not closed under the operator $b$.

\emph{Proof.} Let $L=\{a^nb^n$ $|$ $n \geq 0\}\in \NSIG$. Then $L^b=\{a^n$ $|$ $n\geq 0\}
\notin \NSIG$.
\end{lmm}

Other examples that show \NSIG\ is not closed under the operator $b$ abound. If
$K=\{x\in\{\pmb{a},\pmb{b}\}^*$ $|$ $\ |x|_a = |x|_b\}$, then
$K^b=\{\pmb{a},\pmb{b}\}^*$. The last operator we will cover in this chapter is
useful for illustrating closures that may not be effective, that is, for which
...
$A^/=\langle\Sigma,S_1,s_{0_2},\delta_1,F^/\rangle$, where $F^/$ is defined to
...
actually proves that the quotient of a FAD language with any other language
...
\item Let $\Sigma$ be an alphabet. Define $C_\Sigma$ to be the collection of all
 \index{cofinite!collection $C_\Sigma$} languages over $\Sigma$ (a language is cofinite if it is the complement
...
\item Define $E$ to be the collection of all
...
\item If no minimization is performed in Exercise 5.14, how do the number of
...
words $L$)} \]
$P(L)$ then represents all the prefixes of words in $L$. For example, if
...
words $L$)} \]
$S(L)$ then represents all the suffixes of words in $L$. For example, if
$K=\{\pmb{a},\pmb{bbc},\pmb{dd}\}$, then
...
$C(L)$ then represents all the centers of words in $L$. For example, if
...
\[ F(L)=\{x\ \ | \ x \in L \wedge (\text{if }\exists y \in \Sigma^* \ni xy \in L,
	\text{then }y = \lambda)\} \]
$F(L)$ then represents all the words in $L$ that are not the beginnings of
...
operator $G$ by
\[ G(L)=\{a_na_{n-1}\ldots a_3a_2a_1a_1a_2a_3\ldots a_{n-1}a_n\ |\ a_1a_2a_3\ldots
	a_{n-1}a_n \in L\} \]
\[ =\{w^r \cdot w\ |\ w \in L\} \]
...
$L(A^\bullet) = L_1 \cdot L_2$. Warning! This involves a lot of tedious details.

\item Refer to the construction in Theorem \ref{thm-5.5} and prove that
$L(A_*) = L^*$. Warning! This involves a lot of tedious details.
...
\[ \overline{\delta^\cup}(s,x_) = \left\{\begin{tabular}{lrl}
...
\[ (\forall s \in S_1)(\forall t \in S_2)(\forall x \in \Sigma^*)(\overline{\delta^\cap}
	(\langle s,t\rangle,x) = \langle\delta_1(s,x),\delta_2(t,x)\rangle ) \]
...
\item Let $P=\{x$ $|$ $|x|$ is prime$\}$ and let $I(L)$ be defined by
$I(L) = L \cap P$.
\begin{enumerate}
...
$L'$ then represents the last halves of all the words in $L$. For example, if
$K=\{\pmb{ad},\pmb{abaa},\pmb{ccccc}\}$, then $K=\{\pmb{d},\pmb{aa}\}$. Prove
...
of final states to be the set of ``midway'' states.
