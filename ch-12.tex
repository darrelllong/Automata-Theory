...
There \textcolor{red}{are} an infinite number of DFAs about which the question can
...
questions will be called \emph{undecidable}. An undecidable question about Pascal
...
exist. Such an algorithm could be programmed as one big {\tt case} statement, which
...
halt and answer NO (in a finite amount of time) because there \textcolor{red}{are} an infinite
...
circumstances (that is, to produce an \emph{algorithm}).
...
consist of computing the intersection of $\{\DBAR(s_0,y)\ |\ y\in\Sigma^*\wedge n
...
transition table for $A$ to actually determine $\{\DBAR(s_0,y)\ |\ y\in\Sigma^*
...
Successive sets can thereby be calculated from $R_0$. When $R_n$ is reached, it
...
% Figure 12.1
\begin{figure}[tbh]
\centerline{\includegraphics[width=6in]{ToFAfigures/fig12-1.eps}}
...
dashed box shown in Figure \ref{fig-12.3}. This new machine is very definitely
...
% Figure 12.2
\begin{figure}[tbh]
\centerline{\includegraphics[width=3.5in]{ToFAfigures/fig12-2.eps}}
%The field \ref{ex-12.2} resolves to '12.1" for some reason...
%\caption{The DFAs discussion in Example \ref{ex-12.2}}\label{fig-12.2}
\caption{The automata discussed in Example 12.2}\label{fig-12.2}
\end{figure}

% Figure 12.3
\begin{figure}[tbh]
\centerline{\includegraphics[width=3.5in]{ToFAfigures/fig12-3.eps}}
%The field \ref{ex-12.2} resolves to '12.1" for some reason...
%\caption{The composite DFA discussed in \ref{ex-12.2}}\label{fig-12.3}
\caption{The composite machine discussed in Example 12.2}\label{fig-12.3}
\end{figure}
...
number of \emph{final} states (perhaps there is an equivalent DFA with many
...
in the derivation of \emph{any} valid terminal string. Essentially, only two things can
...
Finding the members of $\Omega$ that \emph{can} be produced from $S$ is a simple
...
a point where $W_n=W_{n+1}$ and $W_n$ will indeed be the set of all nonterminals
...
%The field \ref{ex-12.3} resolves to '12.1" for some reason...
%corresponding to the grammar in Example \ref{ex-12.3} is shown in Figure
corresponding to the grammar in Example 12.3 is shown in Figure
...
% Figure 12.4
\begin{figure}
\centerline{\includegraphics[height=7in]{ToFAfigures/fig12-4.eps}}
...
and contain all words in $L(G)$ of length $\leq n$\textcolor{red}{, If} we are only interested in
...
over some finite character set. As such, there \textcolor{red}{are} only a countable number of
...
involving a countable number of instances. We will argue that there \textcolor{red}{are} an
...
are also finite, and thus there \textcolor{red}{are} a countable number of DFAs, a countable
...
Since there \textcolor{red}{are} an uncountable number of real numbers between 0 and 1, there \textcolor{red}{are}
...
After all, there \textcolor{red}{are} an uncountable number of real numbers, yet all computers
...
\textbf{Case 1}: HALT returns a value of TRUE to the calling program TEST. This has
...
\textbf{Case 2}: HALT returns a value of FALSE to the calling program TEST. This
...
TEST shows that if HALT returns FALSE we execute the {\tt else} statement, write one
...
is \emph{guaranteed} to behave inappropriately when fed a program such as TEST. In
actuality, there \textcolor{red}{are} an infinite number of programs that cause HALT to misbehave,
...
We now consider languages whose \textcolor{red}{criteria} for membership is related to the
...
have shown that, since there \textcolor{red}{are} only a countable number of type 0 grammars and
...
Our original \textcolor{red}{criteria} for belonging to the language $L$ accepted by a Turing
...
\textcolor{red}{$A$} set $L$ is cofinite \emph{iff} its complement is finite, that is, \emph{iff}
$\Sigma^*-L$ is finite.)
...
show that it is decidable whether there is a DFA with \textcolor{red}{less} than 31 states that
accepts the language described by $R_1$.

\item Given any alphabet $\Sigma$ and \textcolor{red}{a} regular expressions $R_1$ over $\Sigma$,
...
\begin{enumerate}
\item
Show that it is decidable, given an arbitrary DFA $A$,
whether at least one of the equivalence classes of $R_{L(A)}$ is finite in size.
\item
Show that it is decidable, given an arbitrary DFA $A$,
whether all of the equivalence classes of $R_{L(A)}$ are finite in size.
\end{enumerate}

\item
Given two right-linear grammars $G_1$ and $G_2$,
show that it is decidable whether
$L(G_1) \subset L(G_2)$.
(Note that this is $\subset$, not $\subseteq$.)
