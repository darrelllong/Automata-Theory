...
There are infinitely many DFAs about which the question can
...
halt and answer NO (in a finite amount of time) because there are infinitely
many strings in $\Sigma^*$ that must be checked. A modification of this
...
over some finite character set. As such, there is only a countable number of
...
involving a countable number of instances. We will argue that there is an
...
are also finite, and thus there is a countable number of DFAs, a countable
...
Since there is an uncountable number of real numbers between 0 and 1, there is
...
After all, there is an uncountable number of real numbers, yet all computers
...
actuality, there is an infinite number of programs that cause HALT to misbehave,
...
We now consider languages whose criteria for membership are related to the
...
have shown that, since there is only a countable number of type 0 grammars and
...
Our original criterion for belonging to the language $L$ accepted by a Turing
