% Chapter, Section, Figure, Table, Theorem, Corollary, Lemma, Definition
...
% Figure 7.1
\begin{figure}
\centerline{\includegraphics[width=6in]{ToFAfigures/fig7-1.eps}}
...
\item $s_0$ denotes the \Index{start state}; $s_0 \in S$.
...
candy bar. $\pmb{\varphi},\pmb{n'},\pmb{d'},\pmb{q'},\pmb{c}_0,\pmb{c}_1,
...
% Figure 7.2
\begin{figure}
\centerline{\includegraphics[width=5.2in]{ToFAfigures/fig7-2.eps}}
...
$ (\forall \pmb{c} \in \Sigma)[\omega(s_0,\pmb{c})=\pmb{0}\quad\text{and}\quad\omega(s_1,\pmb{c}) =
	\pmb{1}] $
% Table 7.1b
...
% Figure 7.3
\begin{figure}
\centerline{\includegraphics[width=2in]{ToFAfigures/fig7-3.eps}}
...
which likewise operated on single \emph{letters}, to the extended state transition
...
$\DBAR$ retains the same conceptual meaning it had for finite-state acceptors:
...
\[ (\forall x \in \Sigma^*)(\forall y \in \Sigma^*)(\forall s \in S)(\DBAR(s,yx)
	=\DBAR(\DBAR(s,y),x)) \]
A similar statement can be made about $\OBAR$ once it has been rigorously
defined.
...
Given a FST $A=\langle\Sigma,\Gamma,S,s_0,\delta,\omega\rangle$, the \emph{extended
...
\item $(\forall t \in S)\qquad\qquad\qquad\qquad\ \ \ \thinspace\thinspace\OBAR
	(t,\lambda)=\lambda$
\item $(\forall t \in S)(\forall x \in \Sigma^*)(\forall \pmb{a} \in \Sigma)
...
the exercises) that (iii) ($\forall t \in S)(\forall \pmb{a} \in \Sigma)(\OBAR(t, \pmb{a})
= \omega(t,\pmb{a}))$, which is the observation that $\omega$ and \OBAR\ treat single
...
% Figure 7.4
\begin{figure}
\centerline{\includegraphics[width=2.5in]{ToFAfigures/fig7-4.eps}}
...
% Figure 7.5
\begin{figure}
\centerline{\includegraphics[width=2.5in]{ToFAfigures/fig7-5.eps}}
...
$D=$\linebreak$\langle\{\pmb{a},\pmb{b}\},\{\pmb{0},\pmb{1}\},\{q_0,q_1,q_2\},q_0,\delta_D,
\omega_D\rangle$, displayed in Figure \ref{fig-7.5}. The function $\mu$$:\{t_0,t_1,t_2,t_3\}
...
$S^c=\{s \in S \mid \exists x \in \Sigma^* \ni \DBAR(s_0,x) = s\}$ \\
$s^c_0 = s_0$
\end{tabular}\end{center}
...
\[ (\forall y \in \Sigma^n)(\forall t \in S^c)(\overline{\omega^c}(t,y) = \OBAR(t,y)). \]
...
$\OBAR(t,z)$ &&
\end{tabular}\end{center}
Since $z$ was an arbitrary element of $\Sigma^{m+1}$\textcolor{red}{ ,} and $t$ was an arbitrary
state in $S^c$,
\[ (\forall z \in \Sigma^{m+1})(\forall t \in S^c)(\overline{\omega^c}(t,z)=\OBAR(t,z)), \]
...
\rangle$, where
\begin{center}\begin{tabular}{rcl}
$S_{E_M}$ & $=$ & $\{[s]_{E_M} \mid s \in S\}$ \\
$s_{0_{E_M}}$ & $=$ & $[s_0]_{E_M}$
\end{tabular}\end{center}
$\delta_{E_M}$ is defined by
\[ (\forall \pmb{a} \in \Sigma)(\forall [s]_{E_M} \in S_{E_M})(\delta_{E_M}([s]_{E_M},
	\pmb{a}) = [\delta(s,\pmb{a})]_{E_M}), \]
and $\omega_{E_M}$ is defined by
\[ (\forall \pmb{a} \in \Sigma)(\forall [s]_{E_M} \in S_{E_M})(\omega_{E_M}([s]_{E_M},
	\pmb{a}) = \omega(s,\pmb{a})). \]
\end{dfn}
...
$x_s$, such that $\DBAR_1(s_{0_1},x_s) = s$. Let $\mu(s)=\DBAR_2(s_{0_2},x_s)$. At
...
state $s$, but all will yield the same value for $\DBAR_2(s_{0_2},x_s)$, and
...
connected machines, $f_{M_1}=f_{M_2}$ \emph{iff} $M_1 \cong M_2$. The conclusions
...
\[ (\forall s \in S)(\forall t \in S)(\forall i \geq 1)(s E_{i+1M} t
	\Leftrightarrow s E_{iM} t \wedge (\forall \pmb{a} \in \Sigma)(\delta(s,
...
$E_{1M}$ until $E_{iM}=E_{i+1M}$; by Lemma \ref{lmm-7.3}, this $E_{iM}$ will equal $E_M$,
...
\section{Moore Sequential Machines}\label{sec-7.3}
...
% Figure 7.6
\begin{figure}
\centerline{\includegraphics[width=2.5in]{ToFAfigures/fig7-6.eps}}
\caption{The state transition diagram for the transducer discussed in Example
7.10}\label{fig-7.6}
\end{figure}
...
function $\OBAR$$: S \times \Sigma^* \rightarrow \Gamma^*$ defined recursively by:
\begin{enumerate}[i. ]
% handspaced vs. hardcoding i., ii.
\item $(\forall t \in S)\qquad\qquad\qquad\qquad\ \ \ \thinspace\thinspace\OBAR
	(t,\lambda)=\lambda$
...
Note that the domain of the function $\omega$ has been extended further than usual:
...
\[ (\forall \pmb{a} \in \Sigma)(\forall s \in S)(\omega'(s,\pmb{a})=
	\omega(\delta(s,\pmb{a}))) \]
\end{dfn}
...
% Figure 7.7
\begin{figure}
\centerline{\includegraphics[width=2.5in]{ToFAfigures/fig7-7.eps}}
...
A translation $f$ is FTD iff $f$ can be defined by a FST $M$ iff $f$ can be
defined by a MSM $A$.
...
% Figure 7.8
\begin{figure}
\centerline{\includegraphics[width=2.5in]{ToFAfigures/fig7-8.eps}}
...
% Figure 7.9
\begin{figure}
\centerline{\includegraphics[width=2.5in]{ToFAfigures/fig7-9.eps}}
\caption{The state transition diagram for the Moore machine $A'$ in Example
7.12}\label{fig-7.9}
\end{figure}
...
machine isomorphism}} from $A$ to $B$ iff the following five conditions hold:\index{isomorphism!between MSMs}
\begin{enumerate}[i.]
\item $\mu(s_{0_A}) = s_{0_B}$.
\item $(\forall s \in S_A)(\forall\pmb{a}\in\Sigma)(\mu(\delta_A(s,\pmb{a})) =
	\delta_B(\mu(s),\pmb{a}))$.
...
\ref{fig-7.9} are \emph{not} isomorphic. In fact, there does not even exist a homomorphism (in
...
\section{Transducer Applications and Circuit Implementation}\label{sec-7.4}
...
% Figure 7.10
\begin{figure}
\centerline{\includegraphics[width=2.5in]{ToFAfigures/fig7-10.eps}}
...
% Figure 7.11
\begin{figure}
\centerline{\includegraphics[width=5.5in]{ToFAfigures/fig7-11.eps}}
...
% Figure 7.12
\begin{figure}
\centerline{\includegraphics[width=5.5in]{ToFAfigures/fig7-12.eps}}
...
% Figure 7.13
\begin{figure}
\centerline{\includegraphics[width=2.5in]{ToFAfigures/fig7-13.eps}}
...
% Figure 7.14
\begin{figure}
\centerline{\includegraphics[width=3.5in]{ToFAfigures/fig7-14.eps}}
...
% Figure 7.15
\begin{figure}
\centerline{\includegraphics[width=3.5in]{ToFAfigures/fig7-15.eps}}
...
For example, incorporating $s_8$ as shown in Figure \ref{fig-7.16} guarantees that the
...
% Figure 7.16
\begin{figure}
\centerline{\includegraphics[width=3.0in]{ToFAfigures/fig7-16.eps}}
...
light in deference to the north-south traffic. In contrast, it \emph{is} important for
...
Consider the FST displayed in Figure \ref{fig-7.17}. If \<EOS\> and \<SOS\> \textcolor{red}{is}
...
% Figure 7.17
\begin{figure}
\centerline{\includegraphics[width=2.5in]{ToFAfigures/fig7-17.eps}}
...
% Figure 7.18
\begin{figure}
\centerline{\includegraphics[width=4.5in]{ToFAfigures/fig7-18.eps}}
...
A larger output alphabet would require an encoding of several bits; each $w_i$
...
\begin{enumerate}
\item $E_{0M}$ has just one equivalence \textcolor{red}{classes}, which consists of all of $S$.
...
\item \begin{enumerate}
\item Give an example of a FST $A$ for which $A$ is \emph{not} reduced and $A^c$
is not reduced.
\item Give an example of a FST $A$ for which $A$ is \emph{not} reduced and $A^c$
\emph{is} reduced.
\end{enumerate}
...
\pmb{1}$, and let $f_1(\pmb{a}_1\pmb{a}_2\ldots a_m) = n^m$ otherwise. Thus,
...
\pmb{1}$, and let $f_2(\pmb{a}_1\pmb{a}_2\ldots \pmb{a}_m)=n^m$ otherwise. Thus,
$f_2(\pmb{10})=\pmb{nn}$ and $f_2(\pmb{0101})=\pmb{yyyy}$. Prove that $f_2$ is
\emph{not} FTD.
\end{enumerate}

\item Let $\Sigma=\{\pmb{a},\pmb{b}\}$ and $\Gamma=\{\pmb{0},\pmb{1}\}$. Define
$f_3(\pmb{a}_1\pmb{a}_2\ldots \pmb{a}_m)$ to be the first $m$ letters of the infinite
...
% Figure 7.19
\begin{figure}
\centerline{\includegraphics[width=4.5in]{ToFAfigures/fig7-19.eps}}
...
% Figure 7.20
\begin{figure}
\centerline{\includegraphics[width=4.5in]{ToFAfigures/fig7-20.eps}}
...
\item Show that homomorphism is \emph{not} an equivalence relation over the set of all
Mealy machines.
\end{enumerate}
