...
%%------------------------------------------------------------
\item \textbf{Define a one-state DPDA $P'_1$ accepting $\{\pmb{a}^n\pmb{b}^n \mid n\geq 1\}$ via final state.}
...
\delta(t,\pmb{a},A) &= \langle t, AA\rangle \quad\text{(push another $A$ for each \pmb{a})} \\
\delta(t,\pmb{b},A) &= \langle t, \lambda\rangle \quad\text{(pop $A$ on reading \pmb{b})} \\
\delta(t,\lambda,Z) &= \langle t, Z\rangle \quad\text{(accept: stack has only $Z$, input exhausted)}
\end{align*}
The machine pushes one $A$ for each $\pmb{a}$ read, then pops one $A$ for each $\pmb{b}$ read. It accepts when the stack returns to just $Z$ and input is empty. $\Lambda(P'_1) = \{\pmb{a}^n\pmb{b}^n \mid n\geq 1\}$.
...
\item From (a), processing $\pmb{a}^n$ from start symbol $S$ results in stack $C^n$ (taking the path that never re-expands $S$, i.e., always choosing $\delta(t,\pmb{a},S)=\langle t,C\rangle$ after the first step, or more precisely: $SC^n$ or $C^n$). For the machine to accept via final state (returning to state $t$ with empty input), the stack must be emptied. From (b), $C^n$ is emptied by reading exactly $\pmb{b}^n$. Thus:
\[
L(P'_2) = \{\pmb{a}^n\pmb{b}^n \mid n\geq 1\}.
\]
For $n=0$: the initial configuration $\langle t,\lambda,S\rangle$ has $S$ still on the stack, so the empty-stack acceptance condition fails and $\lambda\notin L(P'_2)$. Hence $L(P'_2) = \{\pmb{a}^n\pmb{b}^n \mid n\geq 1\}$.
\end{enumerate}
...
\textbf{Example}: $L = \{\pmb{ab}\}$ (just one string).

\textbf{Proof}: Consider any one-state PDA $P$ with state $\{t\}$, final state $\{t\}$. On input $\pmb{ab}$, $P$ must accept: $\langle t,\pmb{ab},Z\rangle\Dvdash\langle t,\lambda,\alpha\rangle$ for some $\alpha$ (not necessarily empty; acceptance via final state does not require empty stack).

But also, if $P$ accepts $\pmb{ab}$, consider what happens on $\pmb{aab}$: when reading $\pmb{a}$ in state $t$ with some stack top, the PDA's move is fixed (same transition as before). This may force $P$ to also accept $\pmb{aab}$ (or fail to distinguish), because with only one state, the stack is the only memory and the transitions are determined by (state, input, stack top). It can be shown that with one state, any language accepted via final state is a CFL of a very restricted form (in fact, any one-state PDA language is context-free, but not every finite language can be represented with exactly one state without also accepting extra strings).

Concretely: a one-state PDA accepting $\pmb{ab}$ would need a stack-based mechanism that is so constrained that it either accepts more strings or fails to properly isolate $\pmb{ab}$. For instance, $\{\pmb{a}\}$ cannot be recognized by a one-state PDA since the PDA cannot simultaneously block $\pmb{aa}$ while accepting $\pmb{a}$.

A standard example: $\{\pmb{ab},\pmb{ba}\}$ --- a two-element language where the ordering of symbols varies. A one-state PDA cannot distinguish between them without nontrivial stack interaction, which forces acceptance of additional strings.
...
\textbf{Case 2}: $vx$ contains $\pmb{b}$s and $\pmb{a}$s (on the left boundary). Let $|vx|_{\pmb{a}}=\ell$, $|vx|_{\pmb{b}}=j$. The pumped string has $k = p!+p + (i-1)\ell$ and $n = p + (i-1)j$. For $i=0$: $k = p!+p - \ell$, $n = p-j$. For some $i$, we can make $k = n$ or $n = m$. Detailed case analysis leads to a contradiction. \qed
