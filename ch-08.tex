...
$\alpha$ to be replaced by another string $\beta$. Thus, $\pmb{aBcD} \rightarrow
\pmb{bcA}$ would be a legal production, and thus whenever the sequence
$\pmb{aBcD}$ is found within a derivation it can be replaced by the shorter
string $\pmb{bcA}$.
...
\pmb{bc}A$, in which four symbols are replaced by only three, will at least
...
length 7 into a string of length 6. Gil does not satisfy the definition of a
...
nonterminal $B$ is replaced by the string $\beta$ in the context of the strings
...
$\Sigma$ is a (nonempty) set of \emph{terminal symbols} (and $\Omega\cap\Sigma =
	\emptyset$). \\
$S$ is the designated \emph{start symbol} (and $S\in\Omega$). \\
$P$ is a set of productions of the form $A\rightarrow\beta$, where $A\in\Omega,
...
$\Sigma$ is a (nonempty) set of \emph{terminal symbols} (and $\Omega\cap\Sigma =
	\emptyset$). \\
$S$ is the designated \emph{start symbol} (and $S\in\Omega$). \\
$P$ is a set of productions of the form $A\rightarrow xB$, where $A\in\Omega, B
...
\Sigma,S_2,P_2\rangle$ are called equivalent iff $L(G_1)=L(G_2)$, and we will
write $G_2 \simeq G_1$.
\end{dfn}
...
expression, is equivalent to another representation $Y$ \emph{iff} $L(X)=L(Y)$.
...
$q_{i_n}\rightarrow\lambda,\ q_0\Rightarrow\pmb{a}_1\pmb{a}_2\cdots\pmb{a}_n
...
% Figure 8.1
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig8-1.eps,width=3in}}
...
% Figure 8.2
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig8-2.eps,width=6.2in}}
...
$\Sigma$ is a (nonempty) set of \emph{terminal symbols} (and $\Omega\cap\Sigma =
	\emptyset$). \\
$S$ is the designated \emph{start symbol} (and $S\in\Omega$). \\
$P$ is a set of productions of the form $A\rightarrow Bx$, where $A\in\Omega,B
	\in(\Omega\cup\lambda)$, and $x\in\Sigma^*$.
\end{quote}
\end{dfn}
...
and $A=A_{nn}$ to find a minimal solution for X, for the corresponding values of
...
only result can be a word from $L(G2)$.
\end{thm}
...
state transition function of $AG$ could have been defined more easily. Other
...
by a series of productions involving newly introduced nonterrninals, for example,
...
% Figure 8.3
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig8-3.eps,width=5.2in}}
...
\item Why was it possible to assume that $\Omega_1\cap\Omega_2=\emptyset$ in the
...
% Figure 8.4
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig8-4.eps,width=5.2in}}
\caption{Automata for Exercise 8.31}\label{fig-8.4}
\end{figure}

% Figure 8.5
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig8-5.eps,width=5.2in}}
...
\[ G=\langle\{A,B,S\},\{\pmb{a},\pmb{b},\pmb{c}\},S,\{S\rightarrow\pmb{a}SB
	...
	\pmb{c}\}\rangle \]
Show that this context-sensitive grammar is not equivalent to $G''$ given in
Example 8.3, where
