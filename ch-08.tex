...
\section{Overview \textcolor{red}{Of} \textcolor{red}{The} Grammar Hierarchy}\label{sec-8.1}
...
\item If $R_1$ and $R_2$ are regular expressions, then so is $\pmb{(}R_1 \textcolor{red}{\pmb\cdot} R_2\pmb{)}$.
\item If $R_1$ and $R_2$ are regular expressions, then so is $\pmb{(}R_1 \textcolor{red}{\pmb\cup} R_2\pmb{)}$.
...
\textcolor{red}{comprised of} $\{S,S_1,S_2,S_3,S_4,S_5\}$. The start symbol designates which of
...
requirement that $Z\not in\Omega\cup\Sigma$ means that the other productions will
...
\rightarrow\pmb{b}T,T\rightarrow\pmb{aa}\}\rangle$\textcolor{red}{.Then} $S\ \DRightarrow\pmb{aabaa}$,
since by Definition \ref{dfn-8.8}, with $x_1=\lambda,\ x_2=\pmb{a},\ x_3=\pmb{aa},\ x_4 =
...
since $S\not in\Sigma$, this is not a valid string of terminals. The only way to
...
$G_8 \sim eq G_1$. The concept of equivalence applies to all types of grammars,
...
letter, and hence $\pmb{ab}\not in L(G_B)$. Since $S$ is not a final state, it
...
% Figure 8.1
\begin{figure}
\centerline{\includegraphics[width=3in]{ToFAfigures/fig8-1.eps}}
...
\end{tabular}\end{center}
and $\delta_G$ is \textcolor{red}{comprised of} (normal) transitions of the form
\begin{center}\begin{tabular}{r@{ }l}
...
% Figure 8.2
\begin{figure}
\centerline{\includegraphics[width=6.2in]{ToFAfigures/fig8-2.eps}}
...
\[ L(G_2)=\pmb{a}^*\pmb{baa}=\{\pmb{baa},\pmb{abaa},\pmb{aabaa},\ldots\} =
	L(G_1), \]
and so $G_2 \sim eq G_1$ (compare with Example 8.7). Note that there does not
...
nonterminal $Z$ such that $Z \not in \Omega_1\cup\Omega_2$, and consider the new
...
% Figure 8.3
\begin{figure}
\centerline{\includegraphics[width=5.2in]{ToFAfigures/fig8-3.eps}}
...
symbol. This is only necessary if $\lambda\in L$; if $\lambda\textcolor{red}{\not in} L$, there
...
\pmb{1}F\}\rangle$. $L(Q)$ generates the set of all (terminating) binary numbers
including \textbf{101.11}, \textbf{011.}, \textbf{10.0}, \textbf{0.010}, and so on.
\begin{enumerate}
...
% Figure 8.4
\begin{figure}
\centerline{\includegraphics[width=5.2in]{ToFAfigures/fig8-4.eps}}
\caption{Automata for Exercise 8.31}\label{fig-8.4}
\end{figure}

% Figure 8.5
\begin{figure}
\centerline{\includegraphics[width=5.2in]{ToFAfigures/fig8-5.eps}}
