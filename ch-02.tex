...
single word over the ASCII alphabet. We might define the language
Pascal as the set of all valid Pascal programs (that is, the valid
...
\emph{reflexive, symmetric}, and \emph{transitive}. The identity
...
Note that the above theorem is very broad in scope: any language, no matter
...
% Figure 2.1
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig2-1.eps,width=4in}}
\caption{(a) The automaton for $L_1$ in Example \ref{fig-2.4} (b) The automaton for $L_2$
in Example \ref{fig-2.4}}\label{fig-2.1}
\end{figure}
...
% Figure 2.2
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig2-2.eps,width=2.5in}}
...
% Figure 2.3
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig2-3.eps,width=2.5in}}
...
3 of the equivalence class of a right congruence of rank 4 (which is finite).
...
% Figure 2.4
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig2-4.eps,width=3.0in}}
...
% Figure 2.5
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig2-5.eps,width=2.5in}}
...
% Figure 2.6
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig2-6.eps,width=4.0in}}
...
$\DBAR(q,v) = q$. Also, $\delta(s,x) = \DBAR(s,uvw) =$ f and $\DBAR(q,w)= f$, as
...
% Figure 2.7
\begin{figure}
\centerline{\epsfig{figure=ToFAfigures/fig2-7.eps,width=4.0in}}
...
uv^i\!w)=\text{f}=\DBAR(s,uvw))$. Notice that we do reach $q$ two distinct times,
...
pumping lemma is generally not applied to FAD languages! (\emph{Note:} We will
see an application later.) The pumping lemma is often applied to show languages
are not FAD (by proving that the language does not satisfy the pumping lemma).
...
required by Theorem \ref{thm-2.4}. For any combination of $u,\ v,\ w \in \Sigma^*$ such
...
$k \geq 1$, and $j + k + m = p$. If $i = 0$, we have that $uv^{\circ} w =
...
original reasoning while assuming the string $x$ takes us to a nonfinal state,
...
Notice that Theorem \ref{thm-2.6} guarantees that if one ``long'' string is not in the
...
\subseteq \Sigma^* \times \Sigma^*$ defined by
\[ x\ Q\ y \Leftrightarrow |x|_a - |y|_b \equiv 0 \bmod{3} \]
\begin{enumerate}
...
\item Define $L=[\lambda]_Q \cup [\pmb{aa}]_Q$. Find a simple decryption for
...
\item Find $R_A$ for each machine $A$ you built in the exercises of Chapter \ref{ch-1};
compare $R_A$ to $R_{L(A)}$.
...
\{\pmb{dd},\pmb{ddd},\pmb{ddddd},\pmb{d}^7,\pmb{d}^{11},\pmb{d}^{l3}\ldots\}$.
Prove that $P$ is not FAD.

\item Let $\Gamma=\{x \in \{\pmb{0},\pmb{1},\pmb{2}\}^*\ |\ \exists w\in\{\pmb{0},
\pmb{1}\}^* \ni = w \cdot 2 \cdot w\} = \{\pmb{2},\pmb{121},\pmb{020},
...
\pmb{a}_3\pmb{a}_4 \ldots \pmb{a}_{n-1}\pmb{a}_n$, then $w'=\pmb{a}_n
\pmb{a}_{n-1} \ldots \pmb{a}_4\pmb{a}_3\pmb{a}_2\pmb{a_1}$. Let
$K=\{w \in \{\pmb{0},\pmb{1}\}^*\ |\ w = w'\} = \{\lambda,\pmb{0},\pmb{1},\pmb{00},
\pmb{11},\pmb{000},\pmb{010},\pmb{101},\pmb{111},\pmb{0000},\pmb{0110},\ldots\}$.
Prove $K$ is not FAD.

\item Let $\Phi=\{x \in \{\pmb{a},\pmb{b},\pmb{c}\}^*\ |\ \exists j,k,m \in \NN \ni
= \pmb{a}^j\pmb{b}^k\pmb{c}^m$, where $j \geq 3$ and $k = m\}$. Prove $\Phi$ is
...
$R_{\{x|\ |x|\text{ is odd}\}})$.
\item List the languages $L$ that could give rise to this $R_L$.
\end{enumerate}
...
$\pmb{b}\}$. Also assume that $L$ consists of exactly one of these equivalence
classes.
\begin{enumerate}
\item Which equivalence class is $L$?
\item List the other languages $L$ that could give rise to this $R_L$ (and note
that they might consist of several equivalence classes).
\end{enumerate}
...
\item Let $I=\{\pmb{a},\pmb{b}\}$ and let $\Gamma=\{\lambda,\pmb{a},\pmb{ab},
...
\item Prove that the relation $R_M$ given in Definition \ref{dfn-2.4} is a right
congruence.
...
\pmb{b}^k\pmb{c}^i\ |\  n < 3$ \emph{ or } $(n \geq 3$ and $k = j)\}$.
\begin{enumerate}
\item Show that for this language the conclusion of Theorem \ref{thm-2.3} holds, but the
hypothesis of Theorem \ref{thm-2.3} does not hold.
...
\item \begin{enumerate}
\item Give an example of a DFA $M$ for which $R_M = R_{L(M)}$.
\item Give an example of a DFA $M$ for which $R_M \not= R_{L(M)}$.
\item For every DFA $M$, show that $R_M$ refines $R_{L(M)}$.
\end{enumerate}

\item Find $R_M$ and $R_{L(M)}$ for the machine $M$ described in Example 1.5.

\item Is $A_{R_{L(A)}}$ always equivalent to $A$? Explain.

\item Consider $L_4=\{y \in \{\pmb{0},\pmb{1}\}^*\,|\ \ |y|_{\pmb{0}} =
|y|_{\pmb{1}}$ as given in Example 2.11. Let $n$ be given and consider $x =
(\pmb{01})^n = \pmb{010101} \ldots \pmb{0101}$. Then $|x| = 2n > n$; but if
