...
$\pmb{ab}u \not in K$ and $\pmb{bbaa}u \not in K$ (when $|u|$ is even) or
...
The \textcolor{red}{ranks} of the relation in Example 2.3 was 2, since there were two equivalence
...
while the relation $R^Y$ identifies $Y$ as a \emph{machine} (since \textcolor{red}{Y} occurs as a
superscript).
...
% Figure 2.1
\begin{figure}
\centerline{\includegraphics[width=4in]{ToFAfigures/fig2-1.eps}}
...
% Figure 2.2
\begin{figure}
\centerline{\includegraphics[width=2.5in]{ToFAfigures/fig2-2.eps}}
\caption{The DFA discussed in Example 2.5}\label{fig-2.2}
\end{figure}

% Theorem 2.2
\begin{thm}\label{thm-2.2}
\textbf{Nerode's Theorem}. Let $L$ be a language over an alphabet $\Sigma$; the
...
most one equivalence class per state, and there \textcolor{red}{are} a finite number of states,
...
\textbf{Case 1}: It could be that $L$ is the empty union; that is, that
...
\textbf{Case 2}: In the nontrivial case, $L$ is the union of one or more of the
...
either (both $z \in L$ and $y \in L$) or (neither $z$ nor $y$ \textcolor{red}{belong} to $L$)
...
% Figure 2.3
\begin{figure}
\centerline{\includegraphics[width=2.5in]{ToFAfigures/fig2-3.eps}}
...
% Figure 2.4
\begin{figure}
\centerline{\includegraphics[width=3.0in]{ToFAfigures/fig2-4.eps}}
...
% Figure 2.5
\begin{figure}
\centerline{\includegraphics[width=2.5in]{ToFAfigures/fig2-5.eps}}
...
% Figure 2.6
\begin{figure}
\centerline{\includegraphics[width=4.0in]{ToFAfigures/fig2-6.eps}}
...
% Figure 2.7
\begin{figure}
\centerline{\includegraphics[width=4.0in]{ToFAfigures/fig2-7.eps}}
...
\textbf{The Pumping Lemma.} Let $L$ be an FAD language over $\Sigma^*$. Then
...
that $uv^iw \not in L_4$. Since $|uv| \leq n$ and the first $n$ letters of $x$ are
...
$uv^2w = \pmb{0}^{n+k} \pmb{1}^n \not in L_4$ (why?). Thus, by Theorem \ref{thm-2.4}, $L_4$ is not FAD
...
since there \textcolor{red}{are} an infinite number of primes). Let $x=\pmb{a}^p\pmb{b}^{(p-1)!}$.
...
$uv^{0} w \not in L$ (why?). Therefore, Theorem \ref{thm-2.3} has been violated, so $L$
...
interest as they do not distinguish between simple and complex languages: \emph{any}
...
\begin{enumerate}
\item Show that $Q$ is a right congruence\textcolor{red}{. and} list the equivalence classes.
...
that are composed of \textcolor{red}{less} than 1 million characters. Is Short Pascal FAD? Any
