...
uv^iw)=\text{f}=\DBAR(s,uvw))$. Notice that we do reach $q$ two \emph{distinct} times,
...
$|v|$ & $\geq$ & $1$
\end{tabular}\end{center}
and $(\forall i \in \NN)(\DBAR(s_0,uv^iw)=$f$)$, that is, $(\forall i \in \NN)
(uv^iw \in L(A))$. In other words, given any ``long'' string in $L(A)$, there is
