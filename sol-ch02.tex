...
\item The eight languages: each union of some subset of $\{[\lambda]_Q, C_e, C_o\}$. The ones satisfying part (e) are those where the machine $A_Q$ with those final states gives $A_{R_K}\cong A_Q$ (i.e., the machine is minimal). This happens when $K$ is $\emptyset$, $\Sigma^*$, or the complement of one class.
\end{enumerate}
...
$F_{R_L}=\{[x]_{R_L}\mid x\in L\}$. We must show: if $[x]_{R_L}=[y]_{R_L}$ and $x\in L$, then $y\in L$. Since $[x]_{R_L}=[y]_{R_L}$, we have $x\,R_L\,y$, meaning $(\forall z\in\Sigma^*)(xz\in L\Leftrightarrow yz\in L)$. Taking $z=\lambda$: $x\in L\Leftrightarrow y\in L$. So if $x\in L$ then $y\in L$. $\square$
...
%%------------------------------------------------------------
\item \textbf{Show the language $I$ (identity relation on $\{\mathbf{a}\}^*$) gives no FAD language.}

\begin{enumerate}
\item $L$ giving rise to $I$: $R_L=I$ requires every pair of distinct strings to have different futures. For instance, $L=\{\mathbf{a}\}$ over $\{\mathbf{a}\}^*$ has $R_L$ with only 3 classes: $\{\lambda\}$, $\{\mathbf{a}\}$, and $\mathbf{a}^{\geq 2}$. A language with $R_L=I$ must have infinitely many classes — one per string.

\item Such a language cannot be FAD (Nerode's theorem: infinite rank $\Rightarrow$ not FAD).
\end{enumerate}
...
A string $x$ is characterized by: (a) $|x|_\mathbf{0}$ (at least for small values), (b) parity of $|x|_\mathbf{1}$.

Class analysis: The condition $|y|_\mathbf{0}=1$ or $|y|_\mathbf{1}$ even is complex. Track state as pair (number of 0s seen, capped at 2, parity of 1s). Classes of $R_\Omega$:
\begin{itemize}
\item $|x|_\mathbf{0}=0$, even \#1: future needs: if $z$ has exactly one 0 and even \#1, or \#1 is even.
\item etc.
\end{itemize}
More carefully: $x\,R_\Omega\,y$ iff for all $z$, $xz\in\Omega\Leftrightarrow yz\in\Omega$. The future of $x$ depends on $(|x|_\mathbf{0}\text{ capped at 2}, |x|_\mathbf{1}\bmod 2)$. This gives at most $3\times 2=6$ classes (and some may merge). The exact classes require detailed analysis of the DFA for $\Omega$.
...
\begin{enumerate}
\item $\mathbf{ab}\,R_\Gamma\,\mathbf{ba}$: For all $z$: $\mathbf{ab}z\in\Gamma\Leftrightarrow\mathbf{ba}z\in\Gamma$. If $|z|=0$: both in $\Gamma$. If $|z|\geq 1$: $|\mathbf{ab}z|=2+|z|\geq 3$. For $|z|=1$: $|\mathbf{ab}z|=3$, and $\mathbf{abz}\in\Gamma$ iff $\mathbf{abz}\in\{x:|x|=3\}\setminus\{\mathbf{bbb}\}$... checking: $\mathbf{aba}$, $\mathbf{abb}$, $\mathbf{baa}$, $\mathbf{bab}$, $\mathbf{bba}$ are all in $\Gamma$ (since $|\cdot|=3$ and they're not $\mathbf{bbb}$). $\mathbf{bbb}$: $\mathbf{ab}\cdot z$ can't equal $\mathbf{bbb}$. So both $\mathbf{abz}$ and $\mathbf{baz}$ are in $\Gamma$. For $|z|\geq 2$: $|\cdot|\geq 4$, automatically in $\Gamma$. So yes, $\mathbf{ab}\,R_\Gamma\,\mathbf{ba}$.
...
$\langle\text{simple}\rangle ::= \mathbf{a}\mid\mathbf{(}\langle\text{simple}\rangle\mathbf{)}$. The language is $\{\mathbf{(}^n\mathbf{a}\mathbf{)}^n\mid n\geq 0\}$. By pumping lemma (or Myhill-Nerode): the strings $\mathbf{(}^0,\mathbf{(}^1,\ldots,\mathbf{(}^n$ are in distinct $R_L$-classes (future of $\mathbf{(}^k$ is $\{\mathbf{a}^{\mathbf{)}^k}\}$ essentially). Not FAD. $\square$
...
\item $L_{10}=\{1,2,4,8,16,32,\ldots\}$ in decimal: by Myhill-Nerode. The strings $1,2,4,\ldots,2^n$ for large $n$ in decimal: these are all distinct $n$-digit (roughly) numbers, and their futures differ (since appending digits to get another power of 2 from $2^k$ is different from $2^j$). More precisely: for any two powers $2^i$ and $2^j$ ($i\neq j$), they are in different $R_{L_{10}}$-classes, since $2^i\cdot z=2^i$ (numerically, but the string concatenation $\langle 2^i\rangle z$ for decimal strings is different from $\langle 2^j\rangle z$). The formal argument: the strings $\langle 2^0\rangle=\mathbf{1},\langle 2^1\rangle=\mathbf{2},\ldots$ are pairwise not $R_{L_{10}}$-related (since $\langle 2^k\rangle\lambda=\langle 2^k\rangle$ is in $L_{10}$ but differs per $k$, and appending $\lambda$ distinguishes them). So infinitely many classes. Not FAD. $\square$
\end{enumerate}
