...
Since $A$ was the only stack symbol in $P_3$, the language could just as easily
been described by the sack-and-stone counting device outlined at the beginning
...
where $\delta_G$ is composed of the following nonempty transitions:
\begin{center}\begin{tabular}{r@{ = }l}
...
Figures \ref{fig-10.5a}-f\, illustrate the state of the machine at several points during the
...
the first criterion for productions. Since $\delta(t,\pmb{a},S)$ contains
...
rule due to the second criterion arises because $\delta(t,\pmb{a},S)$ contains
...
from the first criterion for productions. The transition $\delta(q,\pmb{a},B) =
...
the criterion that a final state can be reached after processing all the letters
on the input tape, rather than the criterion that the stack is emptied. Theorem
...
is not closed under union if $\Sigma$ is composed of at least two symbols
...
Let $\Sigma$ be an alphabet composed of at least two symbols. Then \ASIG\ is
